...
export class Categories2Component implements OnInit {
  ...
  updateTotals(risk:string){
   ...
   if(this.risk_factors.Exposure==1){
    title=title+'Exposure '
   }
   if(this.risk_factors.Health_resources==1){
    title=title+'Health Resources '
   }
   if(this.risk_factors.Socioeconomic==1){
    title=title+'Socioeconomic '
   }
   if(this.risk_factors.Susceptibility==1){
  ...
  cat_geoJSON= L.geoJSON(this.map_obj, {
...
highlightFeature(e) {
  ...
  cat_info.update(layer.feature.properties);
}
...
resetHighlight(e) {
  cat_geoJSON.resetStyle(e.target);
  cat_info.update();
}
