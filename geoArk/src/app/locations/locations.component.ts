import { Component, OnInit } from '@angular/core';
import { HttpHeaders } from '@angular/common/http';
import { HttpClient } from '@angular/common/http';
import { environment} from 'src/environments/environment';
import { NgxSpinnerService } from 'ngx-spinner';
import { Router, ActivatedRoute, Params } from '@angular/router';


//Third Party Packages 
import * as L from 'leaflet';
import * as colormap from 'colormap';
import { trimTrailingNulls } from '@angular/compiler/src/render3/view/util';
import { stringify } from '@angular/compiler/src/util';



//Risk factors map and geoJSON object
var risk_map;
var geoJSON;
var info;
var map_status;

@Component({
  selector: 'app-locations',
  templateUrl: './locations.component.html',
  styleUrls: ['./locations.component.css']
})
export class LocationsComponent implements OnInit {



  

// Look up object for fips and counties
  public counties:any;

  public current_risk_factor:any='susceptibility'

  //Current county-- variables needed to pull in data from database 
  public county_name:any;
  public county_fips:any;

  //County Quick  Facts
  public county_pop:string;
  public county_65:string;
  public county_cov_case:string;
  public county_cov_deaths:string;
  public county_total_risk:string;


  // Covid deaths and cases graphs
  public covid_county_cases:any={};
  public covid_county_deaths:any={};
  public covid_cases:any={};
  public covid_deaths:any={};

  public covid:any=[];
  public update_date:any;



  // Spinner 
  public spinnertogg:boolean=true;
  public spinnertogg2:boolean=false;
  public initial_covid_plots:boolean=true;




  //Risk Factors Graphs
  public risk_factors_bars:any=[];
  public risk_factors:any=[];

  public current_description:any;




  //Risk Factors Map
  public factors:any;
  //public factors_meta:any;

  public current_factor:any='total';

//missouri geoJSON object
  public map_obj:any=[{'type': 'FeatureCollection',
  'features': [{'properties': {'GEO_ID': '0500000US29097',
     'STATE': '29',
     'COUNTY': '097',
     'NAME': 'Jasper',
     'LSAD': 'County',
     'CENSUSAREA': 638.487,
     'fips': 29097},
    'type': 'Feature',
    'geometry': {'type': 'Polygon',
     'coordinates': [[[-94.082083, 37.349291],
       [-94.083833, 37.290851],
       [-94.052313, 37.290078],
       [-94.059211, 37.048127],
       [-94.617875, 37.056797],
       [-94.617875, 37.056798],
       [-94.617982, 37.075077],
       [-94.61812, 37.085934],
       [-94.618082, 37.086432],
       [-94.61809, 37.093494],
       [-94.618088, 37.093671],
       [-94.618059, 37.096676],
       [-94.618151, 37.103968],
       [-94.618212, 37.113169],
       [-94.618075, 37.129755],
       [-94.618072, 37.132345],
       [-94.618351, 37.160211],
       [-94.618473, 37.174782],
       [-94.618505, 37.181184],
       [-94.618319, 37.188774],
       [-94.618305, 37.207337],
       [-94.618219, 37.207772],
       [-94.61815, 37.228121],
       [-94.618123, 37.229334],
       [-94.618158, 37.237597],
       [-94.618075, 37.240436],
       [-94.617648, 37.323589],
       [-94.617695, 37.336842],
       [-94.617636, 37.338415],
       [-94.617636, 37.338417],
       [-94.617538, 37.364167],
       [-94.082083, 37.349291]]]},
    'id': '29097'},
   {'properties': {'GEO_ID': '0500000US29103',
     'STATE': '29',
     'COUNTY': '103',
     'NAME': 'Knox',
     'LSAD': 'County',
     'CENSUSAREA': 504.011,
     'fips': 29103},
    'type': 'Feature',
    'geometry': {'type': 'Polygon',
     'coordinates': [[[-92.345064, 40.03721],
       [-92.347801, 40.302756],
       [-91.949747, 40.300832],
       [-91.950812, 40.25721],
       [-91.952726, 39.949426],
       [-92.177328, 39.954494],
       [-92.287882, 39.95249],
       [-92.344708, 39.953297],
       [-92.345064, 40.03721]]]},
    'id': '29103'},
   {'properties': {'GEO_ID': '0500000US29109',
     'STATE': '29',
     'COUNTY': '109',
     'NAME': 'Lawrence',
     'LSAD': 'County',
     'CENSUSAREA': 611.737,
     'fips': 29109},
    'type': 'Feature',
    'geometry': {'type': 'Polygon',
     'coordinates': [[[-94.059211, 37.048127],
       [-94.052313, 37.290078],
       [-93.625844, 37.282011],
       [-93.605095, 37.281482],
       [-93.608899, 37.098153],
       [-93.610126, 36.99581],
       [-93.611003, 36.922831],
       [-94.06274, 36.931775],
       [-94.059211, 37.048127]]]},
    'id': '29109'},
   {'properties': {'GEO_ID': '0500000US29117',
     'STATE': '29',
     'COUNTY': '117',
     'NAME': 'Livingston',
     'LSAD': 'County',
     'CENSUSAREA': 532.329,
     'fips': 29117},
    'type': 'Feature',
    'geometry': {'type': 'Polygon',
     'coordinates': [[[-93.761973, 39.959552],
       [-93.361957, 39.967606],
       [-93.364808, 39.703803],
       [-93.266968, 39.703562],
       [-93.278098, 39.615632],
       [-93.758357, 39.611407],
       [-93.758992, 39.784674],
       [-93.761973, 39.959552]]]},
    'id': '29117'},
   {'properties': {'GEO_ID': '0500000US29131',
     'STATE': '29',
     'COUNTY': '131',
     'NAME': 'Miller',
     'LSAD': 'County',
     'CENSUSAREA': 592.595,
     'fips': 29131},
    'type': 'Feature',
    'geometry': {'type': 'Polygon',
     'coordinates': [[[-92.604976, 38.156815],
       [-92.695886, 38.222031],
       [-92.689834, 38.348472],
       [-92.633756, 38.348082],
       [-92.625189, 38.430778],
       [-92.623858, 38.430639],
       [-92.495185, 38.427829],
       [-92.403484, 38.425331],
       [-92.408407, 38.341344],
       [-92.194476, 38.335536],
       [-92.195675, 38.292237],
       [-92.18537, 38.016338],
       [-92.406275, 38.021179],
       [-92.515979, 38.022336],
       [-92.604976, 38.156815]]]},
    'id': '29131'},
   {'properties': {'GEO_ID': '0500000US29137',
     'STATE': '29',
     'COUNTY': '137',
     'NAME': 'Monroe',
     'LSAD': 'County',
     'CENSUSAREA': 647.653,
     'fips': 29137},
    'type': 'Feature',
    'geometry': {'type': 'Polygon',
     'coordinates': [[[-91.715247, 39.657644],
       [-91.717968, 39.339122],
       [-92.310028, 39.347795],
       [-92.300871, 39.605348],
       [-92.299316, 39.605344],
       [-92.186691, 39.604082],
       [-92.184986, 39.662932],
       [-91.846743, 39.658513],
       [-91.715247, 39.657644]]]},
    'id': '29137'},
   {'properties': {'GEO_ID': '0500000US29151',
     'STATE': '29',
     'COUNTY': '151',
     'NAME': 'Osage',
     'LSAD': 'County',
     'CENSUSAREA': 604.346,
     'fips': 29151},
    'type': 'Feature',
    'geometry': {'type': 'Polygon',
     'coordinates': [[[-91.644729, 38.28851],
       [-92.195675, 38.292237],
       [-92.194476, 38.335536],
       [-92.114977, 38.401937],
       [-92.168883, 38.466812],
       [-92.043489, 38.466479],
       [-92.025857, 38.552017],
       [-91.944238, 38.596925],
       [-91.869489, 38.66394],
       [-91.647171, 38.703396],
       [-91.645546, 38.703678],
       [-91.641376, 38.703939],
       [-91.640372, 38.703792],
       [-91.644729, 38.28851]]]},
    'id': '29151'},
   {'properties': {'GEO_ID': '0500000US29159',
     'STATE': '29',
     'COUNTY': '159',
     'NAME': 'Pettis',
     'LSAD': 'County',
     'CENSUSAREA': 682.222,
     'fips': 29159},
    'type': 'Feature',
    'geometry': {'type': 'Polygon',
     'coordinates': [[[-93.497278, 38.92843],
       [-93.496377, 38.942837],
       [-93.069819, 38.930042],
       [-93.050453, 38.928244],
       [-93.059974, 38.693077],
       [-93.067294, 38.529995],
       [-93.290454, 38.535388],
       [-93.291851, 38.506321],
       [-93.512743, 38.512476],
       [-93.511303, 38.548565],
       [-93.511364, 38.548889],
       [-93.51103, 38.55621],
       [-93.497278, 38.92843]]]},
    'id': '29159'},
   {'properties': {'GEO_ID': '0500000US29169',
     'STATE': '29',
     'COUNTY': '169',
     'NAME': 'Pulaski',
     'LSAD': 'County',
     'CENSUSAREA': 547.099,
     'fips': 29169},
    'type': 'Feature',
    'geometry': {'type': 'Polygon',
     'coordinates': [[[-92.406872, 37.977802],
       [-92.406275, 38.021179],
       [-92.18537, 38.016338],
       [-92.18537, 38.015138],
       [-92.021165, 38.010639],
       [-92.029258, 37.602542],
       [-92.249463, 37.604543],
       [-92.248865, 37.648442],
       [-92.413669, 37.713941],
       [-92.409231, 37.85918],
       [-92.406872, 37.977802]]]},
    'id': '29169'},
   {'properties': {'GEO_ID': '0500000US29179',
     'STATE': '29',
     'COUNTY': '179',
     'NAME': 'Reynolds',
     'LSAD': 'County',
     'CENSUSAREA': 808.478,
     'fips': 29179},
    'type': 'Feature',
    'geometry': {'type': 'Polygon',
     'coordinates': [[[-90.779553, 37.050324],
       [-90.964466, 37.053584],
       [-91.017766, 37.095677],
       [-91.034346, 37.169343],
       [-91.159323, 37.254936],
       [-91.175405, 37.414805],
       [-91.211863, 37.415277],
       [-91.210984, 37.501911],
       [-91.314236, 37.505132],
       [-91.312458, 37.592824],
       [-91.155073, 37.588092],
       [-90.768695, 37.603011],
       [-90.772949, 37.373659],
       [-90.735453, 37.27104],
       [-90.737146, 37.166413],
       [-90.779553, 37.050324]]]},
    'id': '29179'},
   {'properties': {'GEO_ID': '0500000US29195',
     'STATE': '29',
     'COUNTY': '195',
     'NAME': 'Saline',
     'LSAD': 'County',
     'CENSUSAREA': 755.504,
     'fips': 29195},
    'type': 'Feature',
    'geometry': {'type': 'Polygon',
     'coordinates': [[[-92.849224, 39.226218],
       [-92.94165, 39.125892],
       [-92.934569, 39.064547],
       [-93.048471, 38.972418],
       [-93.050453, 38.928244],
       [-93.069819, 38.930042],
       [-93.496377, 38.942837],
       [-93.477233, 39.292796],
       [-93.333337, 39.250012],
       [-93.332252, 39.309606],
       [-93.230079, 39.327877],
       [-93.200708, 39.401787],
       [-93.104485, 39.383656],
       [-93.072915, 39.33552],
       [-92.959801, 39.312526],
       [-92.906142, 39.222339],
       [-92.849224, 39.226218]]]},
    'id': '29195'},
   {'properties': {'GEO_ID': '0500000US29205',
     'STATE': '29',
     'COUNTY': '205',
     'NAME': 'Shelby',
     'LSAD': 'County',
     'CENSUSAREA': 500.864,
     'fips': 29205},
    'type': 'Feature',
    'geometry': {'type': 'Polygon',
     'coordinates': [[[-92.299316, 39.605344],
       [-92.287882, 39.95249],
       [-92.177328, 39.954494],
       [-91.952726, 39.949426],
       [-91.840353, 39.948235],
       [-91.846743, 39.658513],
       [-92.184986, 39.662932],
       [-92.186691, 39.604082],
       [-92.299316, 39.605344]]]},
    'id': '29205'},
   {'properties': {'GEO_ID': '0500000US29211',
     'STATE': '29',
     'COUNTY': '211',
     'NAME': 'Sullivan',
     'LSAD': 'County',
     'CENSUSAREA': 647.981,
     'fips': 29211},
    'type': 'Feature',
    'geometry': {'type': 'Polygon',
     'coordinates': [[[-93.367214, 40.266314],
       [-93.366935, 40.382999],
       [-92.855313, 40.385894],
       [-92.855629, 40.342736],
       [-92.856191, 40.037253],
       [-93.361951, 40.033098],
       [-93.366556, 40.033099],
       [-93.367214, 40.266314]]]},
    'id': '29211'},
   {'properties': {'GEO_ID': '0500000US29221',
     'STATE': '29',
     'COUNTY': '221',
     'NAME': 'Washington',
     'LSAD': 'County',
     'CENSUSAREA': 759.913,
     'fips': 29221},
    'type': 'Feature',
    'geometry': {'type': 'Polygon',
     'coordinates': [[[-90.63998, 38.076548],
       [-90.645135, 37.734813],
       [-91.100017, 37.740012],
       [-91.095765, 38.204083],
       [-90.780185, 38.204112],
       [-90.63998, 38.076548]]]},
    'id': '29221'},
   {'properties': {'GEO_ID': '0500000US29229',
     'STATE': '29',
     'COUNTY': '229',
     'NAME': 'Wright',
     'LSAD': 'County',
     'CENSUSAREA': 681.771,
     'fips': 29229},
    'type': 'Feature',
    'geometry': {'type': 'Polygon',
     'coordinates': [[[-92.252261, 37.472944],
       [-92.251359, 37.059653],
       [-92.685867, 37.067051],
       [-92.686671, 37.481545],
       [-92.252261, 37.472944]]]},
    'id': '29229'},
   {'properties': {'GEO_ID': '0500000US29001',
     'STATE': '29',
     'COUNTY': '001',
     'NAME': 'Adair',
     'LSAD': 'County',
     'CENSUSAREA': 567.324,
     'fips': 29001},
    'type': 'Feature',
    'geometry': {'type': 'Polygon',
     'coordinates': [[[-92.345064, 40.03721],
       [-92.847477, 40.037301],
       [-92.856191, 40.037253],
       [-92.855629, 40.342736],
       [-92.684167, 40.343466],
       [-92.349975, 40.346505],
       [-92.349897, 40.302733],
       [-92.347801, 40.302756],
       [-92.345064, 40.03721]]]},
    'id': '29001'},
   {'properties': {'GEO_ID': '0500000US29009',
     'STATE': '29',
     'COUNTY': '009',
     'NAME': 'Barry',
     'LSAD': 'County',
     'CENSUSAREA': 778.251,
     'fips': 29009},
    'type': 'Feature',
    'geometry': {'type': 'Polygon',
     'coordinates': [[[-94.068898, 36.747818],
       [-94.06274, 36.931775],
       [-93.611003, 36.922831],
       [-93.5738, 36.820032],
       [-93.584282, 36.498896],
       [-93.700171, 36.499135],
       [-93.709956, 36.499179],
       [-93.718893, 36.499178],
       [-93.727552, 36.499055],
       [-93.728022, 36.499037],
       [-93.866758, 36.498789],
       [-93.906128, 36.498718],
       [-93.92184, 36.498718],
       [-93.95919, 36.498717],
       [-93.96392, 36.498717],
       [-94.077089, 36.49873],
       [-94.068898, 36.747818]]]},
    'id': '29009'},
   {'properties': {'GEO_ID': '0500000US29021',
     'STATE': '29',
     'COUNTY': '021',
     'NAME': 'Buchanan',
     'LSAD': 'County',
     'CENSUSAREA': 408.027,
     'fips': 29021},
    'type': 'Feature',
    'geometry': {'type': 'Polygon',
     'coordinates': [[[-94.602257, 39.747195],
       [-94.601246, 39.530372],
       [-94.601326, 39.532138],
       [-95.102888, 39.533347],
       [-95.109304, 39.542285],
       [-95.113557, 39.553941],
       [-95.113077, 39.559133],
       [-95.107454, 39.573843],
       [-95.069592, 39.615106],
       [-95.01531, 39.674262],
       [-95.011103, 39.677572],
       [-94.937924, 39.725471],
       [-94.916932, 39.728186],
       [-94.895268, 39.76321],
       [-94.908267, 39.812172],
       [-94.876544, 39.820594],
       [-94.602819, 39.819608],
       [-94.602257, 39.747195]]]},
    'id': '29021'},
   {'properties': {'GEO_ID': '0500000US29033',
     'STATE': '29',
     'COUNTY': '033',
     'NAME': 'Carroll',
     'LSAD': 'County',
     'CENSUSAREA': 694.621,
     'fips': 29033},
    'type': 'Feature',
    'geometry': {'type': 'Polygon',
     'coordinates': [[[-93.758357, 39.611407],
       [-93.278098, 39.615632],
       [-93.275932, 39.484371],
       [-93.104485, 39.383656],
       [-93.200708, 39.401787],
       [-93.230079, 39.327877],
       [-93.332252, 39.309606],
       [-93.333337, 39.250012],
       [-93.477233, 39.292796],
       [-93.487885, 39.228875],
       [-93.650526, 39.248214],
       [-93.758463, 39.207021],
       [-93.758651, 39.209623],
       [-93.759183, 39.524558],
       [-93.758357, 39.611407]]]},
    'id': '29033'},
   {'properties': {'GEO_ID': '0500000US29043',
     'STATE': '29',
     'COUNTY': '043',
     'NAME': 'Christian',
     'LSAD': 'County',
     'CENSUSAREA': 562.645,
     'fips': 29043},
    'type': 'Feature',
    'geometry': {'type': 'Polygon',
     'coordinates': [[[-92.909336, 36.809178],
       [-93.146854, 36.81382],
       [-93.304359, 36.816866],
       [-93.342041, 36.817301],
       [-93.337451, 36.992494],
       [-93.610126, 36.99581],
       [-93.608899, 37.098153],
       [-93.065274, 37.088694],
       [-92.903273, 37.070651],
       [-92.909336, 36.809178]]]},
    'id': '29043'},
   {'properties': {'GEO_ID': '0500000US29053',
     'STATE': '29',
     'COUNTY': '053',
     'NAME': 'Cooper',
     'LSAD': 'County',
     'CENSUSAREA': 564.765,
     'fips': 29053},
    'type': 'Feature',
    'geometry': {'type': 'Polygon',
     'coordinates': [[[-93.050453, 38.928244],
       [-93.048471, 38.972418],
       [-92.934569, 39.064547],
       [-92.841617, 38.979821],
       [-92.558086, 38.970692],
       [-92.544946, 38.959691],
       [-92.540118, 38.953718],
       [-92.526437, 38.944776],
       [-92.514987, 38.938114],
       [-92.508657, 38.93271],
       [-92.498377, 38.922084],
       [-92.617147, 38.680013],
       [-92.839837, 38.683974],
       [-92.950502, 38.686028],
       [-93.059974, 38.693077],
       [-93.050453, 38.928244]]]},
    'id': '29053'},
   {'properties': {'GEO_ID': '0500000US29065',
     'STATE': '29',
     'COUNTY': '065',
     'NAME': 'Dent',
     'LSAD': 'County',
     'CENSUSAREA': 752.79,
     'fips': 29065},
    'type': 'Feature',
    'geometry': {'type': 'Polygon',
     'coordinates': [[[-91.528797, 37.788989],
       [-91.308311, 37.787186],
       [-91.310655, 37.700048],
       [-91.153345, 37.69734],
       [-91.155073, 37.588092],
       [-91.312458, 37.592824],
       [-91.314236, 37.505132],
       [-91.210984, 37.501911],
       [-91.211863, 37.415277],
       [-91.646626, 37.422731],
       [-91.75504, 37.42411],
       [-91.754795, 37.598768],
       [-91.809105, 37.598863],
       [-91.806219, 37.791286],
       [-91.528797, 37.788989]]]},
    'id': '29065'},
   {'properties': {'GEO_ID': '0500000US29075',
     'STATE': '29',
     'COUNTY': '075',
     'NAME': 'Gentry',
     'LSAD': 'County',
     'CENSUSAREA': 491.422,
     'fips': 29075},
    'type': 'Feature',
    'geometry': {'type': 'Polygon',
     'coordinates': [[[-94.216696, 40.136626],
       [-94.216494, 40.12503],
       [-94.2188, 40.034885],
       [-94.605377, 40.039217],
       [-94.606575, 40.042049],
       [-94.605942, 40.126127],
       [-94.605593, 40.127767],
       [-94.598487, 40.386714],
       [-94.214278, 40.384254],
       [-94.216696, 40.136626]]]},
    'id': '29075'},
   {'properties': {'GEO_ID': '0500000US29085',
     'STATE': '29',
     'COUNTY': '085',
     'NAME': 'Hickory',
     'LSAD': 'County',
     'CENSUSAREA': 399.091,
     'fips': 29085},
    'type': 'Feature',
    'geometry': {'type': 'Polygon',
     'coordinates': [[[-93.065199, 38.062479],
       [-93.072447, 37.902627],
       [-93.182706, 37.904144],
       [-93.187915, 37.802737],
       [-93.574103, 37.813336],
       [-93.573202, 37.828035],
       [-93.568709, 37.914918],
       [-93.512103, 37.913535],
       [-93.504025, 38.071757],
       [-93.497132, 38.072573],
       [-93.065199, 38.062479]]]},
    'id': '29085'},
   {'properties': {'GEO_ID': '0500000US29007',
     'STATE': '29',
     'COUNTY': '007',
     'NAME': 'Audrain',
     'LSAD': 'County',
     'CENSUSAREA': 692.235,
     'fips': 29007},
    'type': 'Feature',
    'geometry': {'type': 'Polygon',
     'coordinates': [[[-92.314471, 39.246454],
       [-92.313836, 39.249986],
       [-92.310028, 39.347795],
       [-91.717968, 39.339122],
       [-91.718467, 39.324505],
       [-91.438235, 39.318716],
       [-91.409033, 39.142542],
       [-91.629598, 39.147853],
       [-91.633998, 39.059057],
       [-92.110387, 39.064204],
       [-92.104374, 39.239809],
       [-92.314471, 39.246454]]]},
    'id': '29007'},
   {'properties': {'GEO_ID': '0500000US29017',
     'STATE': '29',
     'COUNTY': '017',
     'NAME': 'Bollinger',
     'LSAD': 'County',
     'CENSUSAREA': 617.906,
     'fips': 29017},
    'type': 'Feature',
    'geometry': {'type': 'Polygon',
     'coordinates': [[[-89.866627, 37.126227],
       [-89.959576, 37.127353],
       [-89.959196, 37.05438],
       [-90.111247, 37.041205],
       [-90.110354, 37.085871],
       [-90.22323, 37.086533],
       [-90.217657, 37.314967],
       [-90.145699, 37.312091],
       [-90.146763, 37.597434],
       [-89.861123, 37.599286],
       [-89.866627, 37.126227]]]},
    'id': '29017'},
   {'properties': {'GEO_ID': '0500000US29023',
     'STATE': '29',
     'COUNTY': '023',
     'NAME': 'Butler',
     'LSAD': 'County',
     'CENSUSAREA': 694.669,
     'fips': 29023},
    'type': 'Feature',
    'geometry': {'type': 'Polygon',
     'coordinates': [[[-90.678562, 36.92649],
       [-90.258563, 36.922043],
       [-90.204205, 36.772262],
       [-90.14002, 36.69676],
       [-90.148398, 36.630656],
       [-90.220732, 36.497858],
       [-90.228943, 36.497771],
       [-90.339892, 36.498213],
       [-90.494575, 36.498368],
       [-90.495027, 36.498371],
       [-90.50016, 36.498399],
       [-90.576112, 36.498446],
       [-90.57618, 36.498446],
       [-90.592166, 36.679368],
       [-90.628387, 36.679544],
       [-90.627517, 36.810954],
       [-90.662617, 36.811453],
       [-90.678562, 36.92649]]]},
    'id': '29023'},
   {'properties': {'GEO_ID': '0500000US29029',
     'STATE': '29',
     'COUNTY': '029',
     'NAME': 'Camden',
     'LSAD': 'County',
     'CENSUSAREA': 655.923,
     'fips': 29029},
    'type': 'Feature',
    'geometry': {'type': 'Polygon',
     'coordinates': [[[-92.406872, 37.977802],
       [-92.409231, 37.85918],
       [-92.548934, 37.80047],
       [-92.620181, 37.892521],
       [-92.855384, 37.895736],
       [-93.072447, 37.902627],
       [-93.065199, 38.062479],
       [-93.077692, 38.263092],
       [-92.979558, 38.197734],
       [-92.764963, 38.189534],
       [-92.695886, 38.222031],
       [-92.604976, 38.156815],
       [-92.515979, 38.022336],
       [-92.406275, 38.021179],
       [-92.406872, 37.977802]]]},
    'id': '29029'},
   {'properties': {'GEO_ID': '0500000US29035',
     'STATE': '29',
     'COUNTY': '035',
     'NAME': 'Carter',
     'LSAD': 'County',
     'CENSUSAREA': 507.361,
     'fips': 29035},
    'type': 'Feature',
    'geometry': {'type': 'Polygon',
     'coordinates': [[[-90.678562, 36.92649],
       [-90.662617, 36.811453],
       [-91.115926, 36.823672],
       [-91.224825, 36.825679],
       [-91.22283, 36.883857],
       [-91.21715, 37.086197],
       [-91.017766, 37.095677],
       [-90.964466, 37.053584],
       [-90.779553, 37.050324],
       [-90.678562, 36.92649]]]},
    'id': '29035'},
   {'properties': {'GEO_ID': '0500000US29041',
     'STATE': '29',
     'COUNTY': '041',
     'NAME': 'Chariton',
     'LSAD': 'County',
     'CENSUSAREA': 751.181,
     'fips': 29041},
    'type': 'Feature',
    'geometry': {'type': 'Polygon',
     'coordinates': [[[-93.266968, 39.703562],
       [-93.082799, 39.706709],
       [-92.85792, 39.699985],
       [-92.689331, 39.698094],
       [-92.692149, 39.610265],
       [-92.696592, 39.610371],
       [-92.707367, 39.321614],
       [-92.790369, 39.343586],
       [-92.849224, 39.226218],
       [-92.906142, 39.222339],
       [-92.959801, 39.312526],
       [-93.072915, 39.33552],
       [-93.104485, 39.383656],
       [-93.275932, 39.484371],
       [-93.278098, 39.615632],
       [-93.266968, 39.703562]]]},
    'id': '29041'},
   {'properties': {'GEO_ID': '0500000US29045',
     'STATE': '29',
     'COUNTY': '045',
     'NAME': 'Clark',
     'LSAD': 'County',
     'CENSUSAREA': 504.692,
     'fips': 29045},
    'type': 'Feature',
    'geometry': {'type': 'Polygon',
     'coordinates': [[[-91.950812, 40.25721],
       [-91.949747, 40.300832],
       [-91.943114, 40.605913],
       [-91.868401, 40.608059],
       [-91.832481, 40.609797],
       [-91.824826, 40.610191],
       [-91.813968, 40.610526],
       [-91.800133, 40.610953],
       [-91.795374, 40.611101],
       [-91.785916, 40.611488],
       [-91.729115, 40.61364],
       [-91.716769, 40.59853],
       [-91.696359, 40.588148],
       [-91.620071, 40.540817],
       [-91.618028, 40.53403],
       [-91.622362, 40.514362],
       [-91.619486, 40.507134],
       [-91.524612, 40.410765],
       [-91.490977, 40.393484],
       [-91.484507, 40.3839],
       [-91.419422, 40.378264],
       [-91.444833, 40.36317],
       [-91.46214, 40.342414],
       [-91.489868, 40.286048],
       [-91.501435, 40.248874],
       [-91.950812, 40.25721]]]},
    'id': '29045'},
   {'properties': {'GEO_ID': '0500000US29047',
     'STATE': '29',
     'COUNTY': '047',
     'NAME': 'Clay',
     'LSAD': 'County',
     'CENSUSAREA': 397.298,
     'fips': 29047},
    'type': 'Feature',
    'geometry': {'type': 'Polygon',
     'coordinates': [[[-94.335213, 39.183337],
       [-94.389728, 39.193503],
       [-94.46159, 39.120372],
       [-94.523463, 39.148771],
       [-94.607354, 39.113444],
       [-94.589933, 39.140403],
       [-94.591933, 39.155003],
       [-94.596033, 39.157703],
       [-94.601733, 39.159603],
       [-94.600819, 39.456155],
       [-94.597879, 39.455838],
       [-94.210642, 39.454682],
       [-94.212515, 39.207086],
       [-94.289873, 39.237107],
       [-94.335213, 39.183337]]]},
    'id': '29047'},
   {'properties': {'GEO_ID': '0500000US29051',
     'STATE': '29',
     'COUNTY': '051',
     'NAME': 'Cole',
     'LSAD': 'County',
     'CENSUSAREA': 393.75,
     'fips': 29051},
    'type': 'Feature',
    'geometry': {'type': 'Polygon',
     'coordinates': [[[-92.194476, 38.335536],
       [-92.408407, 38.341344],
       [-92.403484, 38.425331],
       [-92.495185, 38.427829],
       [-92.394866, 38.73913],
       [-92.350605, 38.676398],
       [-92.220661, 38.643167],
       [-92.161663, 38.578057],
       [-92.065233, 38.557079],
       [-91.944238, 38.596925],
       [-92.025857, 38.552017],
       [-92.043489, 38.466479],
       [-92.168883, 38.466812],
       [-92.114977, 38.401937],
       [-92.194476, 38.335536]]]},
    'id': '29051'},
   {'properties': {'GEO_ID': '0500000US29055',
     'STATE': '29',
     'COUNTY': '055',
     'NAME': 'Crawford',
     'LSAD': 'County',
     'CENSUSAREA': 742.517,
     'fips': 29055},
    'type': 'Feature',
    'geometry': {'type': 'Polygon',
     'coordinates': [[[-91.100017, 37.740012],
       [-91.146521, 37.740811],
       [-91.153345, 37.69734],
       [-91.310655, 37.700048],
       [-91.308311, 37.787186],
       [-91.528797, 37.788989],
       [-91.525025, 38.152433],
       [-91.534384, 38.152593],
       [-91.535333, 38.210975],
       [-91.367482, 38.209741],
       [-91.095765, 38.204083],
       [-91.100017, 37.740012]]]},
    'id': '29055'},
   {'properties': {'GEO_ID': '0500000US29057',
     'STATE': '29',
     'COUNTY': '057',
     'NAME': 'Dade',
     'LSAD': 'County',
     'CENSUSAREA': 490.012,
     'fips': 29057},
    'type': 'Feature',
    'geometry': {'type': 'Polygon',
     'coordinates': [[[-93.616033, 37.572689],
       [-93.620379, 37.456289],
       [-93.621153, 37.427423],
       [-93.625844, 37.282011],
       [-94.052313, 37.290078],
       [-94.083833, 37.290851],
       [-94.082083, 37.349291],
       [-94.075385, 37.58157],
       [-93.616033, 37.572689]]]},
    'id': '29057'},
   {'properties': {'GEO_ID': '0500000US29061',
     'STATE': '29',
     'COUNTY': '061',
     'NAME': 'Daviess',
     'LSAD': 'County',
     'CENSUSAREA': 563.24,
     'fips': 29061},
    'type': 'Feature',
    'geometry': {'type': 'Polygon',
     'coordinates': [[[-94.216696, 40.136626],
       [-93.764021, 40.132909],
       [-93.761973, 39.959552],
       [-93.758992, 39.784674],
       [-94.205844, 39.788954],
       [-94.203457, 40.034539],
       [-94.2188, 40.034885],
       [-94.216494, 40.12503],
       [-94.216696, 40.136626]]]},
    'id': '29061'},
   {'properties': {'GEO_ID': '0500000US29067',
     'STATE': '29',
     'COUNTY': '067',
     'NAME': 'Douglas',
     'LSAD': 'County',
     'CENSUSAREA': 813.632,
     'fips': 29067},
    'type': 'Feature',
    'geometry': {'type': 'Polygon',
     'coordinates': [[[-92.251359, 37.059653],
       [-92.25016, 37.059753],
       [-92.088708, 37.056248],
       [-92.095982, 36.794011],
       [-92.112346, 36.794231],
       [-92.764869, 36.806097],
       [-92.909336, 36.809178],
       [-92.903273, 37.070651],
       [-92.685867, 37.067051],
       [-92.251359, 37.059653]]]},
    'id': '29067'},
   {'properties': {'GEO_ID': '0500000US29073',
     'STATE': '29',
     'COUNTY': '073',
     'NAME': 'Gasconade',
     'LSAD': 'County',
     'CENSUSAREA': 517.803,
     'fips': 29073},
    'type': 'Feature',
    'geometry': {'type': 'Polygon',
     'coordinates': [[[-91.367482, 38.209741],
       [-91.535333, 38.210975],
       [-91.534384, 38.152593],
       [-91.538016, 38.15249],
       [-91.580252, 38.152793],
       [-91.589527, 38.153249],
       [-91.630554, 38.153519],
       [-91.644893, 38.153679],
       [-91.644729, 38.28851],
       [-91.640372, 38.703792],
       [-91.545313, 38.676882],
       [-91.418637, 38.709778],
       [-91.395134, 38.70799],
       [-91.369192, 38.699324],
       [-91.367482, 38.209741]]]},
    'id': '29073'},
   {'properties': {'GEO_ID': '0500000US29077',
     'STATE': '29',
     'COUNTY': '077',
     'NAME': 'Greene',
     'LSAD': 'County',
     'CENSUSAREA': 675.304,
     'fips': 29077},
    'type': 'Feature',
    'geometry': {'type': 'Polygon',
     'coordinates': [[[-93.07338, 37.414989],
       [-93.065274, 37.088694],
       [-93.608899, 37.098153],
       [-93.605095, 37.281482],
       [-93.625844, 37.282011],
       [-93.621153, 37.427423],
       [-93.182489, 37.417204],
       [-93.158287, 37.416799],
       [-93.07338, 37.414989]]]},
    'id': '29077'},
   {'properties': {'GEO_ID': '0500000US29079',
     'STATE': '29',
     'COUNTY': '079',
     'NAME': 'Grundy',
     'LSAD': 'County',
     'CENSUSAREA': 435.276,
     'fips': 29079},
    'type': 'Feature',
    'geometry': {'type': 'Polygon',
     'coordinates': [[[-93.763324, 40.263988],
       [-93.367214, 40.266314],
       [-93.366556, 40.033099],
       [-93.361951, 40.033098],
       [-93.361957, 39.967606],
       [-93.761973, 39.959552],
       [-93.764021, 40.132909],
       [-93.763324, 40.263988]]]},
    'id': '29079'},
   {'properties': {'GEO_ID': '0500000US29087',
     'STATE': '29',
     'COUNTY': '087',
     'NAME': 'Holt',
     'LSAD': 'County',
     'CENSUSAREA': 462.693,
     'fips': 29087},
    'type': 'Feature',
    'geometry': {'type': 'Polygon',
     'coordinates': [[[-95.552473, 40.261904],
       [-95.552314, 40.264443],
       [-95.180614, 40.261706],
       [-95.071703, 40.260859],
       [-95.04308, 40.130096],
       [-94.993348, 39.900747],
       [-95.00844, 39.900596],
       [-95.01632, 39.898766],
       [-95.02524, 39.8897],
       [-95.02525, 39.877688],
       [-95.027931, 39.871522],
       [-95.035047, 39.866807],
       [-95.042142, 39.864805],
       [-95.085003, 39.861883],
       [-95.107745, 39.868579],
       [-95.132695, 39.875925],
       [-95.181222, 39.899996],
       [-95.301549, 39.983603],
       [-95.30778, 39.990618],
       [-95.30829, 39.999998],
       [-95.312211, 40.009395],
       [-95.521925, 40.24947],
       [-95.552473, 40.261904]]]},
    'id': '29087'},
   {'properties': {'GEO_ID': '0500000US29105',
     'STATE': '29',
     'COUNTY': '105',
     'NAME': 'Laclede',
     'LSAD': 'County',
     'CENSUSAREA': 764.717,
     'fips': 29105},
    'type': 'Feature',
    'geometry': {'type': 'Polygon',
     'coordinates': [[[-92.249463, 37.604543],
       [-92.252261, 37.472944],
       [-92.686671, 37.481545],
       [-92.852107, 37.484083],
       [-92.853481, 37.48397],
       [-92.855384, 37.895736],
       [-92.620181, 37.892521],
       [-92.548934, 37.80047],
       [-92.409231, 37.85918],
       [-92.413669, 37.713941],
       [-92.248865, 37.648442],
       [-92.249463, 37.604543]]]},
    'id': '29105'},
   {'properties': {'GEO_ID': '0500000US29111',
     'STATE': '29',
     'COUNTY': '111',
     'NAME': 'Lewis',
     'LSAD': 'County',
     'CENSUSAREA': 505.042,
     'fips': 29111},
    'type': 'Feature',
    'geometry': {'type': 'Polygon',
     'coordinates': [[[-91.455887, 39.945538],
       [-91.840353, 39.948235],
       [-91.952726, 39.949426],
       [-91.950812, 40.25721],
       [-91.501435, 40.248874],
       [-91.505968, 40.234305],
       [-91.510332, 40.201142],
       [-91.512974, 40.181062],
       [-91.510322, 40.127994],
       [-91.494865, 40.037421],
       [-91.460287, 39.980333],
       [-91.455887, 39.945538]]]},
    'id': '29111'},
   {'properties': {'GEO_ID': '0500000US29119',
     'STATE': '29',
     'COUNTY': '119',
     'NAME': 'McDonald',
     'LSAD': 'County',
     'CENSUSAREA': 539.48,
     'fips': 29119},
    'type': 'Feature',
    'geometry': {'type': 'Polygon',
     'coordinates': [[[-94.618019, 36.667921],
       [-94.618025, 36.66943],
       [-94.61813, 36.701423],
       [-94.618307, 36.76656],
       [-94.068898, 36.747818],
       [-94.077089, 36.49873],
       [-94.098588, 36.498676],
       [-94.100252, 36.49867],
       [-94.110673, 36.498587],
       [-94.111473, 36.498597],
       [-94.519478, 36.499214],
       [-94.55929, 36.499496],
       [-94.617919, 36.499414],
       [-94.617877, 36.514999],
       [-94.617883, 36.517799],
       [-94.617997, 36.53428],
       [-94.617868, 36.53609],
       [-94.617897, 36.536983],
       [-94.617814, 36.577732],
       [-94.617853, 36.59912],
       [-94.617865, 36.606851],
       [-94.617815, 36.612604],
       [-94.618019, 36.667921]]]},
    'id': '29119'},
   {'properties': {'GEO_ID': '0500000US29125',
     'STATE': '29',
     'COUNTY': '125',
     'NAME': 'Maries',
     'LSAD': 'County',
     'CENSUSAREA': 526.979,
     'fips': 29125},
    'type': 'Feature',
    'geometry': {'type': 'Polygon',
     'coordinates': [[[-92.18537, 38.016338],
       [-92.195675, 38.292237],
       [-91.644729, 38.28851],
       [-91.644893, 38.153679],
       [-91.630554, 38.153519],
       [-91.632431, 38.050951],
       [-91.895347, 38.051199],
       [-92.021165, 38.010639],
       [-92.18537, 38.015138],
       [-92.18537, 38.016338]]]},
    'id': '29125'},
   {'properties': {'GEO_ID': '0500000US29129',
     'STATE': '29',
     'COUNTY': '129',
     'NAME': 'Mercer',
     'LSAD': 'County',
     'CENSUSAREA': 453.835,
     'fips': 29129},
    'type': 'Feature',
    'geometry': {'type': 'Polygon',
     'coordinates': [[[-93.774344, 40.577584],
       [-93.770231, 40.577615],
       [-93.750223, 40.57772],
       [-93.742759, 40.577518],
       [-93.737259, 40.577542],
       [-93.728355, 40.577547],
       [-93.722443, 40.577641],
       [-93.690333, 40.577875],
       [-93.677099, 40.578127],
       [-93.668845, 40.578241],
       [-93.661913, 40.578354],
       [-93.659272, 40.57833],
       [-93.656211, 40.578352],
       [-93.597352, 40.579496],
       [-93.566189, 40.580117],
       [-93.56581, 40.580075],
       [-93.56524, 40.580143],
       [-93.560798, 40.580304],
       [-93.558938, 40.580189],
       [-93.556899, 40.580235],
       [-93.553986, 40.580303],
       [-93.548284, 40.580417],
       [-93.528177, 40.580367],
       [-93.527607, 40.580436],
       [-93.524124, 40.580481],
       [-93.466887, 40.580072],
       [-93.465297, 40.580164],
       [-93.441767, 40.579916],
       [-93.374386, 40.580334],
       [-93.366935, 40.382999],
       [-93.367214, 40.266314],
       [-93.763324, 40.263988],
       [-93.774344, 40.577584]]]},
    'id': '29129'},
   {'properties': {'GEO_ID': '0500000US29139',
     'STATE': '29',
     'COUNTY': '139',
     'NAME': 'Montgomery',
     'LSAD': 'County',
     'CENSUSAREA': 536.247,
     'fips': 29139},
    'type': 'Feature',
    'geometry': {'type': 'Polygon',
     'coordinates': [[[-91.260232, 39.139845],
       [-91.264287, 38.992762],
       [-91.264927, 38.843833],
       [-91.415832, 38.8453],
       [-91.418637, 38.709778],
       [-91.545313, 38.676882],
       [-91.640372, 38.703792],
       [-91.641376, 38.703939],
       [-91.645546, 38.703678],
       [-91.647171, 38.703396],
       [-91.633998, 39.059057],
       [-91.629598, 39.147853],
       [-91.409033, 39.142542],
       [-91.260232, 39.139845]]]},
    'id': '29139'},
   {'properties': {'GEO_ID': '0500000US29141',
     'STATE': '29',
     'COUNTY': '141',
     'NAME': 'Morgan',
     'LSAD': 'County',
     'CENSUSAREA': 597.631,
     'fips': 29141},
    'type': 'Feature',
    'geometry': {'type': 'Polygon',
     'coordinates': [[[-92.695886, 38.222031],
       [-92.764963, 38.189534],
       [-92.979558, 38.197734],
       [-93.077692, 38.263092],
       [-93.067294, 38.529995],
       [-93.059974, 38.693077],
       [-92.950502, 38.686028],
       [-92.839837, 38.683974],
       [-92.841871, 38.596792],
       [-92.625189, 38.430778],
       [-92.633756, 38.348082],
       [-92.689834, 38.348472],
       [-92.695886, 38.222031]]]},
    'id': '29141'},
   {'properties': {'GEO_ID': '0500000US29147',
     'STATE': '29',
     'COUNTY': '147',
     'NAME': 'Nodaway',
     'LSAD': 'County',
     'CENSUSAREA': 876.964,
     'fips': 29147},
    'type': 'Feature',
    'geometry': {'type': 'Polygon',
     'coordinates': [[[-95.04308, 40.130096],
       [-95.071703, 40.260859],
       [-95.180614, 40.261706],
       [-95.177708, 40.478903],
       [-95.202264, 40.578528],
       [-95.164058, 40.578017],
       [-95.154499, 40.57786],
       [-95.120829, 40.577413],
       [-95.112222, 40.577228],
       [-95.110663, 40.577206],
       [-95.110303, 40.57716],
       [-95.107213, 40.577116],
       [-95.097607, 40.577168],
       [-95.079742, 40.577007],
       [-95.068921, 40.57688],
       [-94.966491, 40.575839],
       [-94.955134, 40.575669],
       [-94.914896, 40.575068],
       [-94.901451, 40.574877],
       [-94.896801, 40.574738],
       [-94.823758, 40.573942],
       [-94.819978, 40.573714],
       [-94.811188, 40.573532],
       [-94.773988, 40.572977],
       [-94.716665, 40.572201],
       [-94.714925, 40.572201],
       [-94.682601, 40.571787],
       [-94.632035, 40.571186],
       [-94.598487, 40.386714],
       [-94.605593, 40.127767],
       [-94.605942, 40.126127],
       [-95.04308, 40.130096]]]},
    'id': '29147'},
   {'properties': {'GEO_ID': '0500000US29153',
     'STATE': '29',
     'COUNTY': '153',
     'NAME': 'Ozark',
     'LSAD': 'County',
     'CENSUSAREA': 744.972,
     'fips': 29153},
    'type': 'Feature',
    'geometry': {'type': 'Polygon',
     'coordinates': [[[-92.112346, 36.794231],
       [-92.120415, 36.498863],
       [-92.137741, 36.498706],
       [-92.150295, 36.498634],
       [-92.199396, 36.498351],
       [-92.211449, 36.498395],
       [-92.214143, 36.498372],
       [-92.216412, 36.498417],
       [-92.309424, 36.497894],
       [-92.318415, 36.497711],
       [-92.529145, 36.497739],
       [-92.772333, 36.497772],
       [-92.772341, 36.497772],
       [-92.764869, 36.806097],
       [-92.112346, 36.794231]]]},
    'id': '29153'},
   {'properties': {'GEO_ID': '0500000US29157',
     'STATE': '29',
     'COUNTY': '157',
     'NAME': 'Perry',
     'LSAD': 'County',
     'CENSUSAREA': 474.354,
     'fips': 29157},
    'type': 'Feature',
    'geometry': {'type': 'Polygon',
     'coordinates': [[[-89.521523, 37.566208],
       [-89.603574, 37.592056],
       [-89.861123, 37.599286],
       [-90.146763, 37.597434],
       [-90.147306, 37.613546],
       [-90.146778, 37.641618],
       [-90.110537, 37.67134],
       [-89.999768, 37.821744],
       [-89.938191, 37.875111],
       [-89.937383, 37.874693],
       [-89.923185, 37.870672],
       [-89.901832, 37.869822],
       [-89.881475, 37.879591],
       [-89.862949, 37.896906],
       [-89.851048, 37.90398],
       [-89.844786, 37.905572],
       [-89.739873, 37.84693],
       [-89.674304, 37.803179],
       [-89.671255, 37.801144],
       [-89.515436, 37.67137],
       [-89.506563, 37.62505],
       [-89.511936, 37.584564],
       [-89.518007, 37.583962],
       [-89.521274, 37.578971],
       [-89.521407, 37.572143],
       [-89.521523, 37.566208]]]},
    'id': '29157'},
   {'properties': {'GEO_ID': '0500000US29161',
     'STATE': '29',
     'COUNTY': '161',
     'NAME': 'Phelps',
     'LSAD': 'County',
     'CENSUSAREA': 671.784,
     'fips': 29161},
    'type': 'Feature',
    'geometry': {'type': 'Polygon',
     'coordinates': [[[-91.809105, 37.598863],
       [-92.029258, 37.602542],
       [-92.021165, 38.010639],
       [-91.895347, 38.051199],
       [-91.632431, 38.050951],
       [-91.630554, 38.153519],
       [-91.589527, 38.153249],
       [-91.580252, 38.152793],
       [-91.538016, 38.15249],
       [-91.534384, 38.152593],
       [-91.525025, 38.152433],
       [-91.528797, 37.788989],
       [-91.806219, 37.791286],
       [-91.809105, 37.598863]]]},
    'id': '29161'},
   {'properties': {'GEO_ID': '0500000US29167',
     'STATE': '29',
     'COUNTY': '167',
     'NAME': 'Polk',
     'LSAD': 'County',
     'CENSUSAREA': 635.523,
     'fips': 29167},
    'type': 'Feature',
    'geometry': {'type': 'Polygon',
     'coordinates': [[[-93.621153, 37.427423],
       [-93.620379, 37.456289],
       [-93.616033, 37.572689],
       [-93.628404, 37.829435],
       [-93.573202, 37.828035],
       [-93.574103, 37.813336],
       [-93.187915, 37.802737],
       [-93.175222, 37.67489],
       [-93.182489, 37.417204],
       [-93.621153, 37.427423]]]},
    'id': '29167'},
   {'properties': {'GEO_ID': '0500000US29171',
     'STATE': '29',
     'COUNTY': '171',
     'NAME': 'Putnam',
     'LSAD': 'County',
     'CENSUSAREA': 517.317,
     'fips': 29171},
    'type': 'Feature',
    'geometry': {'type': 'Polygon',
     'coordinates': [[[-93.366935, 40.382999],
       [-93.374386, 40.580334],
       [-93.345442, 40.580514],
       [-93.317605, 40.580671],
       [-93.260612, 40.580797],
       [-93.098507, 40.583973],
       [-93.097296, 40.584014],
       [-93.085517, 40.584403],
       [-92.957747, 40.58743],
       [-92.941595, 40.587743],
       [-92.903544, 40.58786],
       [-92.889796, 40.588039],
       [-92.879178, 40.588341],
       [-92.863034, 40.588175],
       [-92.857391, 40.58836],
       [-92.835074, 40.588484],
       [-92.827992, 40.588515],
       [-92.828061, 40.588593],
       [-92.757407, 40.588908],
       [-92.742232, 40.589207],
       [-92.714598, 40.589564],
       [-92.667478, 40.408258],
       [-92.684167, 40.343466],
       [-92.855629, 40.342736],
       [-92.855313, 40.385894],
       [-93.366935, 40.382999]]]},
    'id': '29171'},
   {'properties': {'GEO_ID': '0500000US29175',
     'STATE': '29',
     'COUNTY': '175',
     'NAME': 'Randolph',
     'LSAD': 'County',
     'CENSUSAREA': 482.684,
     'fips': 29175},
    'type': 'Feature',
    'geometry': {'type': 'Polygon',
     'coordinates': [[[-92.692149, 39.610265],
       [-92.300871, 39.605348],
       [-92.310028, 39.347795],
       [-92.313836, 39.249986],
       [-92.314471, 39.246454],
       [-92.430229, 39.248795],
       [-92.707367, 39.321614],
       [-92.696592, 39.610371],
       [-92.692149, 39.610265]]]},
    'id': '29175'},
   {'properties': {'GEO_ID': '0500000US29177',
     'STATE': '29',
     'COUNTY': '177',
     'NAME': 'Ray',
     'LSAD': 'County',
     'CENSUSAREA': 568.805,
     'fips': 29177},
    'type': 'Feature',
    'geometry': {'type': 'Polygon',
     'coordinates': [[[-93.758463, 39.207021],
       [-93.840147, 39.215467],
       [-93.988892, 39.152786],
       [-94.104823, 39.143507],
       [-94.212515, 39.207086],
       [-94.210642, 39.454682],
       [-94.21083, 39.456337],
       [-94.208423, 39.5272],
       [-93.759183, 39.524558],
       [-93.758651, 39.209623],
       [-93.758463, 39.207021]]]},
    'id': '29177'},
   {'properties': {'GEO_ID': '0500000US29181',
     'STATE': '29',
     'COUNTY': '181',
     'NAME': 'Ripley',
     'LSAD': 'County',
     'CENSUSAREA': 629.537,
     'fips': 29181},
    'type': 'Feature',
    'geometry': {'type': 'Polygon',
     'coordinates': [[[-90.662617, 36.811453],
       [-90.627517, 36.810954],
       [-90.628387, 36.679544],
       [-90.592166, 36.679368],
       [-90.57618, 36.498446],
       [-90.585342, 36.498497],
       [-90.5943, 36.498459],
       [-90.60545, 36.498459],
       [-90.612554, 36.498559],
       [-90.648494, 36.498447],
       [-90.653246, 36.498488],
       [-90.693005, 36.49851],
       [-90.711226, 36.498318],
       [-90.765672, 36.498494],
       [-90.782454, 36.498523],
       [-90.784398, 36.498524],
       [-90.850434, 36.498548],
       [-90.873775, 36.498074],
       [-90.876567, 36.498313],
       [-90.876867, 36.498423],
       [-90.87922, 36.498378],
       [-90.960648, 36.498426],
       [-90.963063, 36.498418],
       [-91.008558, 36.49827],
       [-91.017974, 36.498062],
       [-91.09588, 36.49787],
       [-91.096277, 36.497893],
       [-91.126529, 36.497712],
       [-91.115926, 36.823672],
       [-90.662617, 36.811453]]]},
    'id': '29181'},
   {'properties': {'GEO_ID': '0500000US29186',
     'STATE': '29',
     'COUNTY': '186',
     'NAME': 'Ste. Genevieve',
     'LSAD': 'County',
     'CENSUSAREA': 499.153,
     'fips': 29186},
    'type': 'Feature',
    'geometry': {'type': 'Polygon',
     'coordinates': [[[-90.110537, 37.67134],
       [-90.200208, 37.669628],
       [-90.459133, 37.878924],
       [-90.32287, 37.981731],
       [-90.416022, 38.042315],
       [-90.254112, 38.12199],
       [-90.243116, 38.112669],
       [-90.203847, 38.088753],
       [-90.126532, 38.041666],
       [-90.072283, 38.017001],
       [-89.997103, 37.963225],
       [-89.974918, 37.926719],
       [-89.973642, 37.917661],
       [-89.950594, 37.881526],
       [-89.938191, 37.875111],
       [-89.999768, 37.821744],
       [-90.110537, 37.67134]]]},
    'id': '29186'},
   {'properties': {'GEO_ID': '0500000US29187',
     'STATE': '29',
     'COUNTY': '187',
     'NAME': 'St. Francois',
     'LSAD': 'County',
     'CENSUSAREA': 451.887,
     'fips': 29187},
    'type': 'Feature',
    'geometry': {'type': 'Polygon',
     'coordinates': [[[-90.535016, 37.643641],
       [-90.647874, 37.642809],
       [-90.645135, 37.734813],
       [-90.63998, 38.076548],
       [-90.583324, 38.003499],
       [-90.416022, 38.042315],
       [-90.32287, 37.981731],
       [-90.459133, 37.878924],
       [-90.200208, 37.669628],
       [-90.110537, 37.67134],
       [-90.146778, 37.641618],
       [-90.516756, 37.642665],
       [-90.535016, 37.643641]]]},
    'id': '29187'},
   {'properties': {'GEO_ID': '0500000US29197',
     'STATE': '29',
     'COUNTY': '197',
     'NAME': 'Schuyler',
     'LSAD': 'County',
     'CENSUSAREA': 307.303,
     'fips': 29197},
    'type': 'Feature',
    'geometry': {'type': 'Polygon',
     'coordinates': [[[-92.349975, 40.346505],
       [-92.684167, 40.343466],
       [-92.667478, 40.408258],
       [-92.714598, 40.589564],
       [-92.689854, 40.589884],
       [-92.686693, 40.589809],
       [-92.639223, 40.590825],
       [-92.637898, 40.590853],
       [-92.580278, 40.592151],
       [-92.484588, 40.594924],
       [-92.482394, 40.594894],
       [-92.481692, 40.594941],
       [-92.461609, 40.595355],
       [-92.453745, 40.595288],
       [-92.379691, 40.596509],
       [-92.350807, 40.597273],
       [-92.349975, 40.346505]]]},
    'id': '29197'},
   {'properties': {'GEO_ID': '0500000US29203',
     'STATE': '29',
     'COUNTY': '203',
     'NAME': 'Shannon',
     'LSAD': 'County',
     'CENSUSAREA': 1003.82,
     'fips': 29203},
    'type': 'Feature',
    'geometry': {'type': 'Polygon',
     'coordinates': [[[-91.22283, 36.883857],
       [-91.250129, 36.88515],
       [-91.658111, 36.888724],
       [-91.657814, 36.889648],
       [-91.655607, 37.048925],
       [-91.646626, 37.422731],
       [-91.211863, 37.415277],
       [-91.175405, 37.414805],
       [-91.159323, 37.254936],
       [-91.034346, 37.169343],
       [-91.017766, 37.095677],
       [-91.21715, 37.086197],
       [-91.22283, 36.883857]]]},
    'id': '29203'},
   {'properties': {'GEO_ID': '0500000US29207',
     'STATE': '29',
     'COUNTY': '207',
     'NAME': 'Stoddard',
     'LSAD': 'County',
     'CENSUSAREA': 823.222,
     'fips': 29207},
    'type': 'Feature',
    'geometry': {'type': 'Polygon',
     'coordinates': [[[-89.958786, 36.629331],
       [-90.148398, 36.630656],
       [-90.14002, 36.69676],
       [-90.204205, 36.772262],
       [-90.258563, 36.922043],
       [-90.111247, 37.041205],
       [-89.959196, 37.05438],
       [-89.959576, 37.127353],
       [-89.866627, 37.126227],
       [-89.765211, 37.126066],
       [-89.786819, 37.087824],
       [-89.68662, 36.928293],
       [-89.691603, 36.859517],
       [-89.700952, 36.627453],
       [-89.958786, 36.629331]]]},
    'id': '29207'},
   {'properties': {'GEO_ID': '0500000US29213',
     'STATE': '29',
     'COUNTY': '213',
     'NAME': 'Taney',
     'LSAD': 'County',
     'CENSUSAREA': 632.436,
     'fips': 29213},
    'type': 'Feature',
    'geometry': {'type': 'Polygon',
     'coordinates': [[[-93.146854, 36.81382],
       [-92.909336, 36.809178],
       [-92.764869, 36.806097],
       [-92.772341, 36.497772],
       [-92.838621, 36.498079],
       [-92.838876, 36.498033],
       [-92.854049, 36.497983],
       [-92.894001, 36.49785],
       [-92.894336, 36.497867],
       [-93.013742, 36.49813],
       [-93.068455, 36.49825],
       [-93.069512, 36.498242],
       [-93.087635, 36.498239],
       [-93.088988, 36.498184],
       [-93.296117, 36.498389],
       [-93.315324, 36.498408],
       [-93.304359, 36.816866],
       [-93.146854, 36.81382]]]},
    'id': '29213'},
   {'properties': {'GEO_ID': '0500000US29219',
     'STATE': '29',
     'COUNTY': '219',
     'NAME': 'Warren',
     'LSAD': 'County',
     'CENSUSAREA': 428.601,
     'fips': 29219},
    'type': 'Feature',
    'geometry': {'type': 'Polygon',
     'coordinates': [[[-90.9622, 38.7742],
       [-90.964461, 38.547545],
       [-91.077324, 38.609343],
       [-91.226547, 38.621567],
       [-91.313093, 38.706715],
       [-91.369192, 38.699324],
       [-91.395134, 38.70799],
       [-91.418637, 38.709778],
       [-91.415832, 38.8453],
       [-91.264927, 38.843833],
       [-91.264287, 38.992762],
       [-91.18839, 38.992141],
       [-91.189348, 38.932151],
       [-91.110187, 38.931901],
       [-91.111251, 38.872709],
       [-90.958536, 38.870865],
       [-90.9622, 38.7742]]]},
    'id': '29219'},
   {'properties': {'GEO_ID': '0500000US29225',
     'STATE': '29',
     'COUNTY': '225',
     'NAME': 'Webster',
     'LSAD': 'County',
     'CENSUSAREA': 592.562,
     'fips': 29225},
    'type': 'Feature',
    'geometry': {'type': 'Polygon',
     'coordinates': [[[-93.065274, 37.088694],
       [-93.07338, 37.414989],
       [-93.071584, 37.487444],
       [-92.853481, 37.48397],
       [-92.852107, 37.484083],
       [-92.686671, 37.481545],
       [-92.685867, 37.067051],
       [-92.903273, 37.070651],
       [-93.065274, 37.088694]]]},
    'id': '29225'},
   {'properties': {'GEO_ID': '0500000US29510',
     'STATE': '29',
     'COUNTY': '510',
     'NAME': 'St. Louis',
     'LSAD': 'city',
     'CENSUSAREA': 61.909,
     'fips': 29510},
    'type': 'Feature',
    'geometry': {'type': 'Polygon',
     'coordinates': [[[-90.301911, 38.655652],
       [-90.251709, 38.718948],
       [-90.165082, 38.770618],
       [-90.171309, 38.766549],
       [-90.20991, 38.72605],
       [-90.21201, 38.71175],
       [-90.20921, 38.70275],
       [-90.20221, 38.69345],
       [-90.19521, 38.68755],
       [-90.18641, 38.67475],
       [-90.181399, 38.660378],
       [-90.18111, 38.65955],
       [-90.17771, 38.64275],
       [-90.17801, 38.63375],
       [-90.182625, 38.617989],
       [-90.18451, 38.611551],
       [-90.196011, 38.594451],
       [-90.202511, 38.588651],
       [-90.224512, 38.574651],
       [-90.248913, 38.544752],
       [-90.257773, 38.532008],
       [-90.306433, 38.63359],
       [-90.304812, 38.642444],
       [-90.3036, 38.645003],
       [-90.301911, 38.655652]]]},
    'id': '29510'},
   {'properties': {'GEO_ID': '0500000US29095',
     'STATE': '29',
     'COUNTY': '095',
     'NAME': 'Jackson',
     'LSAD': 'County',
     'CENSUSAREA': 604.462,
     'fips': 29095},
    'type': 'Feature',
    'geometry': {'type': 'Polygon',
     'coordinates': [[[-94.607354, 39.113444],
       [-94.523463, 39.148771],
       [-94.46159, 39.120372],
       [-94.389728, 39.193503],
       [-94.335213, 39.183337],
       [-94.289873, 39.237107],
       [-94.212515, 39.207086],
       [-94.104823, 39.143507],
       [-94.109584, 39.018745],
       [-94.109953, 39.015815],
       [-94.110863, 38.986311],
       [-94.111478, 38.982862],
       [-94.112928, 38.960904],
       [-94.11427, 38.92857],
       [-94.114175, 38.917304],
       [-94.113712, 38.915366],
       [-94.119096, 38.833167],
       [-94.120778, 38.833006],
       [-94.399447, 38.838809],
       [-94.608033, 38.847207],
       [-94.608033, 38.855007],
       [-94.608033, 38.861207],
       [-94.607993, 38.867271],
       [-94.608033, 38.868107],
       [-94.608033, 38.869207],
       [-94.608033, 38.883807],
       [-94.607978, 38.93687],
       [-94.607866, 38.937398],
       [-94.608134, 38.940006],
       [-94.608134, 38.942006],
       [-94.607391, 39.044086],
       [-94.607234, 39.065704],
       [-94.607334, 39.081704],
       [-94.607234, 39.089604],
       [-94.607354, 39.113444]]]},
    'id': '29095'},
   {'properties': {'GEO_ID': '0500000US29099',
     'STATE': '29',
     'COUNTY': '099',
     'NAME': 'Jefferson',
     'LSAD': 'County',
     'CENSUSAREA': 656.629,
     'fips': 29099},
    'type': 'Feature',
    'geometry': {'type': 'Polygon',
     'coordinates': [[[-90.736319, 38.466645],
       [-90.591123, 38.501652],
       [-90.405262, 38.501153],
       [-90.376841, 38.452861],
       [-90.340297, 38.386998],
       [-90.346118, 38.381853],
       [-90.350918, 38.375053],
       [-90.370819, 38.333554],
       [-90.372519, 38.323354],
       [-90.373929, 38.281853],
       [-90.363926, 38.236355],
       [-90.353902, 38.213855],
       [-90.334258, 38.189932],
       [-90.254112, 38.12199],
       [-90.416022, 38.042315],
       [-90.583324, 38.003499],
       [-90.63998, 38.076548],
       [-90.780185, 38.204112],
       [-90.737599, 38.415897],
       [-90.736319, 38.466645]]]},
    'id': '29099'},
   {'properties': {'GEO_ID': '0500000US29115',
     'STATE': '29',
     'COUNTY': '115',
     'NAME': 'Linn',
     'LSAD': 'County',
     'CENSUSAREA': 615.564,
     'fips': 29115},
    'type': 'Feature',
    'geometry': {'type': 'Polygon',
     'coordinates': [[[-93.266968, 39.703562],
       [-93.364808, 39.703803],
       [-93.361957, 39.967606],
       [-93.361951, 40.033098],
       [-92.856191, 40.037253],
       [-92.847477, 40.037301],
       [-92.85792, 39.699985],
       [-93.082799, 39.706709],
       [-93.266968, 39.703562]]]},
    'id': '29115'},
   {'properties': {'GEO_ID': '0500000US29121',
     'STATE': '29',
     'COUNTY': '121',
     'NAME': 'Macon',
     'LSAD': 'County',
     'CENSUSAREA': 801.227,
     'fips': 29121},
    'type': 'Feature',
    'geometry': {'type': 'Polygon',
     'coordinates': [[[-92.345064, 40.03721],
       [-92.344708, 39.953297],
       [-92.287882, 39.95249],
       [-92.299316, 39.605344],
       [-92.300871, 39.605348],
       [-92.692149, 39.610265],
       [-92.689331, 39.698094],
       [-92.85792, 39.699985],
       [-92.847477, 40.037301],
       [-92.345064, 40.03721]]]},
    'id': '29121'},
   {'properties': {'GEO_ID': '0500000US29135',
     'STATE': '29',
     'COUNTY': '135',
     'NAME': 'Moniteau',
     'LSAD': 'County',
     'CENSUSAREA': 415.025,
     'fips': 29135},
    'type': 'Feature',
    'geometry': {'type': 'Polygon',
     'coordinates': [[[-92.495185, 38.427829],
       [-92.623858, 38.430639],
       [-92.625189, 38.430778],
       [-92.841871, 38.596792],
       [-92.839837, 38.683974],
       [-92.617147, 38.680013],
       [-92.498377, 38.922084],
       [-92.379614, 38.800076],
       [-92.394866, 38.73913],
       [-92.495185, 38.427829]]]},
    'id': '29135'},
   {'properties': {'GEO_ID': '0500000US29145',
     'STATE': '29',
     'COUNTY': '145',
     'NAME': 'Newton',
     'LSAD': 'County',
     'CENSUSAREA': 624.765,
     'fips': 29145},
    'type': 'Feature',
    'geometry': {'type': 'Polygon',
     'coordinates': [[[-94.068898, 36.747818],
       [-94.618307, 36.76656],
       [-94.61838, 36.84732],
       [-94.618658, 36.880064],
       [-94.618243, 36.897027],
       [-94.618282, 36.911472],
       [-94.618207, 36.926236],
       [-94.618295, 36.929647],
       [-94.618166, 36.937584],
       [-94.618109, 36.946564],
       [-94.618026, 36.950158],
       [-94.618031, 36.994704],
       [-94.618049, 36.996208],
       [-94.61808, 36.998135],
       [-94.617995, 37.009016],
       [-94.617965, 37.040537],
       [-94.617875, 37.056797],
       [-94.059211, 37.048127],
       [-94.06274, 36.931775],
       [-94.068898, 36.747818]]]},
    'id': '29145'},
   {'properties': {'GEO_ID': '0500000US29163',
     'STATE': '29',
     'COUNTY': '163',
     'NAME': 'Pike',
     'LSAD': 'County',
     'CENSUSAREA': 670.438,
     'fips': 29163},
    'type': 'Feature',
    'geometry': {'type': 'Polygon',
     'coordinates': [[[-91.260232, 39.139845],
       [-91.409033, 39.142542],
       [-91.438235, 39.318716],
       [-91.460442, 39.450722],
       [-91.178012, 39.598196],
       [-91.168419, 39.564928],
       [-91.158606, 39.553048],
       [-91.042351, 39.452062],
       [-90.993789, 39.422959],
       [-90.939983, 39.393555],
       [-90.882706, 39.362254],
       [-90.842175, 39.341932],
       [-90.799346, 39.313087],
       [-90.72996, 39.255894],
       [-90.726981, 39.251173],
       [-90.718996, 39.224973],
       [-91.182942, 39.227233],
       [-91.18546, 39.13905],
       [-91.260232, 39.139845]]]},
    'id': '29163'},
   {'properties': {'GEO_ID': '0500000US29173',
     'STATE': '29',
     'COUNTY': '173',
     'NAME': 'Ralls',
     'LSAD': 'County',
     'CENSUSAREA': 469.781,
     'fips': 29173},
    'type': 'Feature',
    'geometry': {'type': 'Polygon',
     'coordinates': [[[-91.715247, 39.657644],
       [-91.715497, 39.686492],
       [-91.313162, 39.684907],
       [-91.229317, 39.620853],
       [-91.178012, 39.598196],
       [-91.460442, 39.450722],
       [-91.438235, 39.318716],
       [-91.718467, 39.324505],
       [-91.717968, 39.339122],
       [-91.715247, 39.657644]]]},
    'id': '29173'},
   {'properties': {'GEO_ID': '0500000US29185',
     'STATE': '29',
     'COUNTY': '185',
     'NAME': 'St. Clair',
     'LSAD': 'County',
     'CENSUSAREA': 669.983,
     'fips': 29185},
    'type': 'Feature',
    'geometry': {'type': 'Polygon',
     'coordinates': [[[-94.049895, 38.213985],
       [-93.523484, 38.207216],
       [-93.504025, 38.071757],
       [-93.512103, 37.913535],
       [-93.568709, 37.914918],
       [-93.573202, 37.828035],
       [-93.628404, 37.829435],
       [-93.811402, 37.834571],
       [-93.808648, 37.892775],
       [-94.065675, 37.900987],
       [-94.058285, 38.036627],
       [-94.049895, 38.213985]]]},
    'id': '29185'},
   {'properties': {'GEO_ID': '0500000US29201',
     'STATE': '29',
     'COUNTY': '201',
     'NAME': 'Scott',
     'LSAD': 'County',
     'CENSUSAREA': 419.99,
     'fips': 29201},
    'type': 'Feature',
    'geometry': {'type': 'Polygon',
     'coordinates': [[[-89.691603, 36.859517],
       [-89.68662, 36.928293],
       [-89.786819, 37.087824],
       [-89.765211, 37.126066],
       [-89.620256, 37.233008],
       [-89.491222, 37.248629],
       [-89.467631, 37.2182],
       [-89.414471, 37.12505],
       [-89.385434, 37.05513],
       [-89.383937, 37.046441],
       [-89.378277, 37.039605],
       [-89.322733, 37.009033],
       [-89.465393, 36.935729],
       [-89.519809, 36.869617],
       [-89.691603, 36.859517]]]},
    'id': '29201'},
   {'properties': {'GEO_ID': '0500000US29215',
     'STATE': '29',
     'COUNTY': '215',
     'NAME': 'Texas',
     'LSAD': 'County',
     'CENSUSAREA': 1177.266,
     'fips': 29215},
    'type': 'Feature',
    'geometry': {'type': 'Polygon',
     'coordinates': [[[-92.252261, 37.472944],
       [-92.249463, 37.604543],
       [-92.029258, 37.602542],
       [-91.809105, 37.598863],
       [-91.754795, 37.598768],
       [-91.75504, 37.42411],
       [-91.646626, 37.422731],
       [-91.655607, 37.048925],
       [-92.088708, 37.056248],
       [-92.25016, 37.059753],
       [-92.251359, 37.059653],
       [-92.252261, 37.472944]]]},
    'id': '29215'},
   {'properties': {'GEO_ID': '0500000US29223',
     'STATE': '29',
     'COUNTY': '223',
     'NAME': 'Wayne',
     'LSAD': 'County',
     'CENSUSAREA': 759.176,
     'fips': 29223},
    'type': 'Feature',
    'geometry': {'type': 'Polygon',
     'coordinates': [[[-90.258563, 36.922043],
       [-90.678562, 36.92649],
       [-90.779553, 37.050324],
       [-90.737146, 37.166413],
       [-90.735453, 37.27104],
       [-90.550603, 37.272608],
       [-90.549729, 37.317594],
       [-90.217657, 37.314967],
       [-90.22323, 37.086533],
       [-90.110354, 37.085871],
       [-90.111247, 37.041205],
       [-90.258563, 36.922043]]]},
    'id': '29223'},
   {'properties': {'GEO_ID': '0500000US29003',
     'STATE': '29',
     'COUNTY': '003',
     'NAME': 'Andrew',
     'LSAD': 'County',
     'CENSUSAREA': 432.703,
     'fips': 29003},
    'type': 'Feature',
    'geometry': {'type': 'Polygon',
     'coordinates': [[[-95.04308, 40.130096],
       [-94.605942, 40.126127],
       [-94.606575, 40.042049],
       [-94.605377, 40.039217],
       [-94.602819, 39.819608],
       [-94.876544, 39.820594],
       [-94.908267, 39.812172],
       [-94.927897, 39.886112],
       [-94.935963, 39.894355],
       [-94.954155, 39.901138],
       [-94.993348, 39.900747],
       [-95.04308, 40.130096]]]},
    'id': '29003'},
   {'properties': {'GEO_ID': '0500000US29015',
     'STATE': '29',
     'COUNTY': '015',
     'NAME': 'Benton',
     'LSAD': 'County',
     'CENSUSAREA': 704.06,
     'fips': 29015},
    'type': 'Feature',
    'geometry': {'type': 'Polygon',
     'coordinates': [[[-93.504025, 38.071757],
       [-93.523484, 38.207216],
       [-93.512743, 38.512476],
       [-93.291851, 38.506321],
       [-93.290454, 38.535388],
       [-93.067294, 38.529995],
       [-93.077692, 38.263092],
       [-93.065199, 38.062479],
       [-93.497132, 38.072573],
       [-93.504025, 38.071757]]]},
    'id': '29015'},
   {'properties': {'GEO_ID': '0500000US29027',
     'STATE': '29',
     'COUNTY': '027',
     'NAME': 'Callaway',
     'LSAD': 'County',
     'CENSUSAREA': 834.575,
     'fips': 29027},
    'type': 'Feature',
    'geometry': {'type': 'Polygon',
     'coordinates': [[[-91.944238, 38.596925],
       [-92.065233, 38.557079],
       [-92.161663, 38.578057],
       [-92.220661, 38.643167],
       [-92.17095, 38.7653],
       [-92.131512, 39.064484],
       [-92.110387, 39.064204],
       [-91.633998, 39.059057],
       [-91.647171, 38.703396],
       [-91.869489, 38.66394],
       [-91.944238, 38.596925]]]},
    'id': '29027'},
   {'properties': {'GEO_ID': '0500000US29039',
     'STATE': '29',
     'COUNTY': '039',
     'NAME': 'Cedar',
     'LSAD': 'County',
     'CENSUSAREA': 474.483,
     'fips': 29039},
    'type': 'Feature',
    'geometry': {'type': 'Polygon',
     'coordinates': [[[-93.616033, 37.572689],
       [-94.075385, 37.58157],
       [-94.073524, 37.639722],
       [-94.065675, 37.900987],
       [-93.808648, 37.892775],
       [-93.811402, 37.834571],
       [-93.628404, 37.829435],
       [-93.616033, 37.572689]]]},
    'id': '29039'},
   {'properties': {'GEO_ID': '0500000US29049',
     'STATE': '29',
     'COUNTY': '049',
     'NAME': 'Clinton',
     'LSAD': 'County',
     'CENSUSAREA': 418.956,
     'fips': 29049},
    'type': 'Feature',
    'geometry': {'type': 'Polygon',
     'coordinates': [[[-94.602257, 39.747195],
       [-94.205961, 39.745841],
       [-94.208423, 39.5272],
       [-94.21083, 39.456337],
       [-94.210642, 39.454682],
       [-94.597879, 39.455838],
       [-94.600819, 39.456155],
       [-94.601246, 39.530372],
       [-94.602257, 39.747195]]]},
    'id': '29049'},
   {'properties': {'GEO_ID': '0500000US29059',
     'STATE': '29',
     'COUNTY': '059',
     'NAME': 'Dallas',
     'LSAD': 'County',
     'CENSUSAREA': 540.77,
     'fips': 29059},
    'type': 'Feature',
    'geometry': {'type': 'Polygon',
     'coordinates': [[[-93.182489, 37.417204],
       [-93.175222, 37.67489],
       [-93.187915, 37.802737],
       [-93.182706, 37.904144],
       [-93.072447, 37.902627],
       [-92.855384, 37.895736],
       [-92.853481, 37.48397],
       [-93.071584, 37.487444],
       [-93.07338, 37.414989],
       [-93.158287, 37.416799],
       [-93.182489, 37.417204]]]},
    'id': '29059'},
   {'properties': {'GEO_ID': '0500000US29071',
     'STATE': '29',
     'COUNTY': '071',
     'NAME': 'Franklin',
     'LSAD': 'County',
     'CENSUSAREA': 922.681,
     'fips': 29071},
    'type': 'Feature',
    'geometry': {'type': 'Polygon',
     'coordinates': [[[-90.964461, 38.547545],
       [-90.916554, 38.534725],
       [-90.732328, 38.639302],
       [-90.736319, 38.466645],
       [-90.737599, 38.415897],
       [-90.780185, 38.204112],
       [-91.095765, 38.204083],
       [-91.367482, 38.209741],
       [-91.369192, 38.699324],
       [-91.313093, 38.706715],
       [-91.226547, 38.621567],
       [-91.077324, 38.609343],
       [-90.964461, 38.547545]]]},
    'id': '29071'},
   {'properties': {'GEO_ID': '0500000US29083',
     'STATE': '29',
     'COUNTY': '083',
     'NAME': 'Henry',
     'LSAD': 'County',
     'CENSUSAREA': 696.947,
     'fips': 29083},
    'type': 'Feature',
    'geometry': {'type': 'Polygon',
     'coordinates': [[[-94.064317, 38.56738],
       [-93.51103, 38.55621],
       [-93.511364, 38.548889],
       [-93.511303, 38.548565],
       [-93.512743, 38.512476],
       [-93.523484, 38.207216],
       [-94.049895, 38.213985],
       [-94.075214, 38.214502],
       [-94.065713, 38.447087],
       [-94.064317, 38.56738]]]},
    'id': '29083'},
   {'properties': {'GEO_ID': '0500000US29093',
     'STATE': '29',
     'COUNTY': '093',
     'NAME': 'Iron',
     'LSAD': 'County',
     'CENSUSAREA': 550.259,
     'fips': 29093},
    'type': 'Feature',
    'geometry': {'type': 'Polygon',
     'coordinates': [[[-90.535016, 37.643641],
       [-90.549729, 37.317594],
       [-90.550603, 37.272608],
       [-90.735453, 37.27104],
       [-90.772949, 37.373659],
       [-90.768695, 37.603011],
       [-91.155073, 37.588092],
       [-91.153345, 37.69734],
       [-91.146521, 37.740811],
       [-91.100017, 37.740012],
       [-90.645135, 37.734813],
       [-90.647874, 37.642809],
       [-90.535016, 37.643641]]]},
    'id': '29093'},
   {'properties': {'GEO_ID': '0500000US29005',
     'STATE': '29',
     'COUNTY': '005',
     'NAME': 'Atchison',
     'LSAD': 'County',
     'CENSUSAREA': 547.302,
     'fips': 29005},
    'type': 'Feature',
    'geometry': {'type': 'Polygon',
     'coordinates': [[[-95.709974, 40.523798],
       [-95.74868, 40.524275],
       [-95.75711, 40.52599],
       [-95.76692, 40.531563],
       [-95.769281, 40.536656],
       [-95.774704, 40.573574],
       [-95.773549, 40.578205],
       [-95.765645, 40.585208],
       [-95.746443, 40.584935],
       [-95.6875, 40.584381],
       [-95.687442, 40.58438],
       [-95.64184, 40.584234],
       [-95.611069, 40.583495],
       [-95.574046, 40.582963],
       [-95.554959, 40.582629],
       [-95.533182, 40.582249],
       [-95.526682, 40.582136],
       [-95.525392, 40.58209],
       [-95.469319, 40.58154],
       [-95.415406, 40.581014],
       [-95.373923, 40.580501],
       [-95.373893, 40.580501],
       [-95.357802, 40.5801],
       [-95.335588, 40.579871],
       [-95.221525, 40.578827],
       [-95.218783, 40.578781],
       [-95.217455, 40.578759],
       [-95.213327, 40.578689],
       [-95.212715, 40.578679],
       [-95.21159, 40.578654],
       [-95.211408, 40.57865],
       [-95.202264, 40.578528],
       [-95.177708, 40.478903],
       [-95.180614, 40.261706],
       [-95.552314, 40.264443],
       [-95.550966, 40.285947],
       [-95.553292, 40.291158],
       [-95.622684, 40.342323],
       [-95.68728, 40.465059],
       [-95.692665, 40.483022],
       [-95.681409, 40.49082],
       [-95.659281, 40.519808],
       [-95.696673, 40.545137],
       [-95.697281, 40.536985],
       [-95.709974, 40.523798]]]},
    'id': '29005'},
   {'properties': {'GEO_ID': '0500000US29011',
     'STATE': '29',
     'COUNTY': '011',
     'NAME': 'Barton',
     'LSAD': 'County',
     'CENSUSAREA': 591.921,
     'fips': 29011},
    'type': 'Feature',
    'geometry': {'type': 'Polygon',
     'coordinates': [[[-94.082083, 37.349291],
       [-94.617538, 37.364167],
       [-94.617537, 37.364355],
       [-94.617626, 37.367445],
       [-94.617625, 37.367576],
       [-94.617557, 37.396375],
       [-94.617511, 37.410909],
       [-94.617265, 37.425536],
       [-94.617132, 37.439818],
       [-94.617201, 37.454788],
       [-94.617205, 37.460373],
       [-94.617222, 37.460476],
       [-94.61718, 37.465203],
       [-94.617183, 37.469665],
       [-94.617023, 37.483765],
       [-94.616789, 37.52151],
       [-94.616908, 37.527804],
       [-94.617186, 37.553485],
       [-94.61716, 37.557308],
       [-94.617081, 37.567013],
       [-94.617315, 37.571499],
       [-94.617283, 37.571896],
       [-94.617428, 37.609522],
       [-94.6173, 37.610495],
       [-94.617477, 37.63717],
       [-94.617575, 37.653577],
       [-94.073524, 37.639722],
       [-94.075385, 37.58157],
       [-94.082083, 37.349291]]]},
    'id': '29011'},
   {'properties': {'GEO_ID': '0500000US29019',
     'STATE': '29',
     'COUNTY': '019',
     'NAME': 'Boone',
     'LSAD': 'County',
     'CENSUSAREA': 685.414,
     'fips': 29019},
    'type': 'Feature',
    'geometry': {'type': 'Polygon',
     'coordinates': [[[-92.430229, 39.248795],
       [-92.314471, 39.246454],
       [-92.104374, 39.239809],
       [-92.110387, 39.064204],
       [-92.131512, 39.064484],
       [-92.17095, 38.7653],
       [-92.220661, 38.643167],
       [-92.350605, 38.676398],
       [-92.394866, 38.73913],
       [-92.379614, 38.800076],
       [-92.498377, 38.922084],
       [-92.508657, 38.93271],
       [-92.514987, 38.938114],
       [-92.526437, 38.944776],
       [-92.540118, 38.953718],
       [-92.544946, 38.959691],
       [-92.558086, 38.970692],
       [-92.565124, 38.998274],
       [-92.430229, 39.248795]]]},
    'id': '29019'},
   {'properties': {'GEO_ID': '0500000US29025',
     'STATE': '29',
     'COUNTY': '025',
     'NAME': 'Caldwell',
     'LSAD': 'County',
     'CENSUSAREA': 426.393,
     'fips': 29025},
    'type': 'Feature',
    'geometry': {'type': 'Polygon',
     'coordinates': [[[-93.758992, 39.784674],
       [-93.758357, 39.611407],
       [-93.759183, 39.524558],
       [-94.208423, 39.5272],
       [-94.205961, 39.745841],
       [-94.205728, 39.756815],
       [-94.205432, 39.760821],
       [-94.205884, 39.767152],
       [-94.205844, 39.788954],
       [-93.758992, 39.784674]]]},
    'id': '29025'},
   {'properties': {'GEO_ID': '0500000US29031',
     'STATE': '29',
     'COUNTY': '031',
     'NAME': 'Cape Girardeau',
     'LSAD': 'County',
     'CENSUSAREA': 578.535,
     'fips': 29031},
    'type': 'Feature',
    'geometry': {'type': 'Polygon',
     'coordinates': [[[-89.491222, 37.248629],
       [-89.620256, 37.233008],
       [-89.765211, 37.126066],
       [-89.866627, 37.126227],
       [-89.861123, 37.599286],
       [-89.603574, 37.592056],
       [-89.521523, 37.566208],
       [-89.521697, 37.557325],
       [-89.517051, 37.537278],
       [-89.492051, 37.494008],
       [-89.475525, 37.471388],
       [-89.450969, 37.450069],
       [-89.439769, 37.4372],
       [-89.42594, 37.407471],
       [-89.421054, 37.387668],
       [-89.428185, 37.356158],
       [-89.432836, 37.347056],
       [-89.43604, 37.344441],
       [-89.447556, 37.340475],
       [-89.474569, 37.338165],
       [-89.484598, 37.334832],
       [-89.489005, 37.333368],
       [-89.511842, 37.310825],
       [-89.518393, 37.289354],
       [-89.517032, 37.28192],
       [-89.491222, 37.248629]]]},
    'id': '29031'},
   {'properties': {'GEO_ID': '0500000US29037',
     'STATE': '29',
     'COUNTY': '037',
     'NAME': 'Cass',
     'LSAD': 'County',
     'CENSUSAREA': 696.836,
     'fips': 29037},
    'type': 'Feature',
    'geometry': {'type': 'Polygon',
     'coordinates': [[[-94.399447, 38.838809],
       [-94.120778, 38.833006],
       [-94.119096, 38.833167],
       [-94.119664, 38.568712],
       [-94.064317, 38.56738],
       [-94.065713, 38.447087],
       [-94.31217, 38.471496],
       [-94.612865, 38.477602],
       [-94.612696, 38.483154],
       [-94.612726, 38.484367],
       [-94.612644, 38.491644],
       [-94.612272, 38.547917],
       [-94.612157, 38.549817],
       [-94.612176, 38.576546],
       [-94.611902, 38.58011],
       [-94.611887, 38.580139],
       [-94.611908, 38.609272],
       [-94.611858, 38.620485],
       [-94.611602, 38.635384],
       [-94.609509, 38.738102],
       [-94.609456, 38.7407],
       [-94.609399, 38.74244],
       [-94.609039, 38.760611],
       [-94.608041, 38.811064],
       [-94.608033, 38.847207],
       [-94.399447, 38.838809]]]},
    'id': '29037'},
   {'properties': {'GEO_ID': '0500000US29101',
     'STATE': '29',
     'COUNTY': '101',
     'NAME': 'Johnson',
     'LSAD': 'County',
     'CENSUSAREA': 829.282,
     'fips': 29101},
    'type': 'Feature',
    'geometry': {'type': 'Polygon',
     'coordinates': [[[-94.114175, 38.917304],
       [-93.836319, 38.908709],
       [-93.834719, 38.937909],
       [-93.497278, 38.92843],
       [-93.51103, 38.55621],
       [-94.064317, 38.56738],
       [-94.119664, 38.568712],
       [-94.119096, 38.833167],
       [-94.113712, 38.915366],
       [-94.114175, 38.917304]]]},
    'id': '29101'},
   {'properties': {'GEO_ID': '0500000US29107',
     'STATE': '29',
     'COUNTY': '107',
     'NAME': 'Lafayette',
     'LSAD': 'County',
     'CENSUSAREA': 628.433,
     'fips': 29107},
    'type': 'Feature',
    'geometry': {'type': 'Polygon',
     'coordinates': [[[-94.109584, 39.018745],
       [-94.104823, 39.143507],
       [-93.988892, 39.152786],
       [-93.840147, 39.215467],
       [-93.758463, 39.207021],
       [-93.650526, 39.248214],
       [-93.487885, 39.228875],
       [-93.477233, 39.292796],
       [-93.496377, 38.942837],
       [-93.497278, 38.92843],
       [-93.834719, 38.937909],
       [-93.836319, 38.908709],
       [-94.114175, 38.917304],
       [-94.11427, 38.92857],
       [-94.112928, 38.960904],
       [-94.111478, 38.982862],
       [-94.110863, 38.986311],
       [-94.109953, 39.015815],
       [-94.109584, 39.018745]]]},
    'id': '29107'},
   {'properties': {'GEO_ID': '0500000US29113',
     'STATE': '29',
     'COUNTY': '113',
     'NAME': 'Lincoln',
     'LSAD': 'County',
     'CENSUSAREA': 626.556,
     'fips': 29113},
    'type': 'Feature',
    'geometry': {'type': 'Polygon',
     'coordinates': [[[-90.71158, 39.046798],
       [-90.66587, 38.934195],
       [-90.77821, 38.922488],
       [-90.817827, 38.875966],
       [-90.958536, 38.870865],
       [-91.111251, 38.872709],
       [-91.110187, 38.931901],
       [-91.189348, 38.932151],
       [-91.18839, 38.992141],
       [-91.264287, 38.992762],
       [-91.260232, 39.139845],
       [-91.18546, 39.13905],
       [-91.182942, 39.227233],
       [-90.718996, 39.224973],
       [-90.681086, 39.10059],
       [-90.682744, 39.088348],
       [-90.712541, 39.057064],
       [-90.713629, 39.053977],
       [-90.71158, 39.046798]]]},
    'id': '29113'},
   {'properties': {'GEO_ID': '0500000US29123',
     'STATE': '29',
     'COUNTY': '123',
     'NAME': 'Madison',
     'LSAD': 'County',
     'CENSUSAREA': 494.39,
     'fips': 29123},
    'type': 'Feature',
    'geometry': {'type': 'Polygon',
     'coordinates': [[[-90.535016, 37.643641],
       [-90.516756, 37.642665],
       [-90.146778, 37.641618],
       [-90.147306, 37.613546],
       [-90.146763, 37.597434],
       [-90.145699, 37.312091],
       [-90.217657, 37.314967],
       [-90.549729, 37.317594],
       [-90.535016, 37.643641]]]},
    'id': '29123'},
   {'properties': {'GEO_ID': '0500000US29127',
     'STATE': '29',
     'COUNTY': '127',
     'NAME': 'Marion',
     'LSAD': 'County',
     'CENSUSAREA': 436.924,
     'fips': 29127},
    'type': 'Feature',
    'geometry': {'type': 'Polygon',
     'coordinates': [[[-91.369953, 39.745042],
       [-91.370009, 39.732524],
       [-91.367753, 39.729029],
       [-91.349039, 39.712316],
       [-91.313162, 39.684907],
       [-91.715497, 39.686492],
       [-91.715247, 39.657644],
       [-91.846743, 39.658513],
       [-91.840353, 39.948235],
       [-91.455887, 39.945538],
       [-91.446385, 39.870394],
       [-91.432919, 39.840554],
       [-91.365396, 39.777266],
       [-91.367962, 39.759124],
       [-91.369953, 39.745042]]]},
    'id': '29127'},
   {'properties': {'GEO_ID': '0500000US29183',
     'STATE': '29',
     'COUNTY': '183',
     'NAME': 'St. Charles',
     'LSAD': 'County',
     'CENSUSAREA': 560.435,
     'fips': 29183},
    'type': 'Feature',
    'geometry': {'type': 'Polygon',
     'coordinates': [[[-90.571822, 38.681052],
       [-90.660509, 38.688551],
       [-90.732328, 38.639302],
       [-90.916554, 38.534725],
       [-90.964461, 38.547545],
       [-90.9622, 38.7742],
       [-90.958536, 38.870865],
       [-90.817827, 38.875966],
       [-90.77821, 38.922488],
       [-90.66587, 38.934195],
       [-90.663372, 38.928042],
       [-90.657254, 38.92027],
       [-90.625122, 38.888654],
       [-90.583388, 38.86903],
       [-90.566557, 38.868847],
       [-90.555693, 38.870785],
       [-90.54403, 38.87505],
       [-90.507451, 38.902767],
       [-90.486974, 38.925982],
       [-90.482725, 38.934712],
       [-90.482419, 38.94446],
       [-90.472122, 38.958838],
       [-90.467784, 38.961809],
       [-90.450809, 38.967758],
       [-90.450792, 38.967764],
       [-90.406367, 38.962554],
       [-90.385751, 38.956848],
       [-90.275932, 38.926453],
       [-90.250248, 38.919344],
       [-90.22585, 38.908923],
       [-90.207035, 38.898954],
       [-90.113327, 38.849306],
       [-90.109407, 38.843548],
       [-90.109107, 38.837448],
       [-90.114707, 38.815048],
       [-90.117707, 38.805748],
       [-90.298743, 38.887759],
       [-90.482299, 38.764348],
       [-90.49579, 38.748256],
       [-90.528024, 38.733702],
       [-90.53134, 38.731178],
       [-90.533638, 38.726871],
       [-90.530378, 38.708195],
       [-90.530363, 38.69863],
       [-90.533785, 38.691915],
       [-90.541612, 38.685267],
       [-90.554722, 38.681552],
       [-90.571822, 38.681052]]]},
    'id': '29183'},
   {'properties': {'GEO_ID': '0500000US29189',
     'STATE': '29',
     'COUNTY': '189',
     'NAME': 'St. Louis',
     'LSAD': 'County',
     'CENSUSAREA': 507.8,
     'fips': 29189},
    'type': 'Feature',
    'geometry': {'type': 'Polygon',
     'coordinates': [[[-90.340297, 38.386998],
       [-90.376841, 38.452861],
       [-90.405262, 38.501153],
       [-90.591123, 38.501652],
       [-90.736319, 38.466645],
       [-90.732328, 38.639302],
       [-90.660509, 38.688551],
       [-90.571822, 38.681052],
       [-90.554722, 38.681552],
       [-90.541612, 38.685267],
       [-90.533785, 38.691915],
       [-90.530363, 38.69863],
       [-90.530378, 38.708195],
       [-90.533638, 38.726871],
       [-90.53134, 38.731178],
       [-90.528024, 38.733702],
       [-90.49579, 38.748256],
       [-90.482299, 38.764348],
       [-90.298743, 38.887759],
       [-90.117707, 38.805748],
       [-90.123107, 38.798048],
       [-90.165082, 38.770618],
       [-90.251709, 38.718948],
       [-90.301911, 38.655652],
       [-90.3036, 38.645003],
       [-90.304812, 38.642444],
       [-90.306433, 38.63359],
       [-90.257773, 38.532008],
       [-90.260314, 38.528352],
       [-90.263064, 38.52022],
       [-90.279215, 38.472453],
       [-90.285215, 38.443453],
       [-90.295316, 38.426753],
       [-90.340297, 38.386998]]]},
    'id': '29189'},
   {'properties': {'GEO_ID': '0500000US29199',
     'STATE': '29',
     'COUNTY': '199',
     'NAME': 'Scotland',
     'LSAD': 'County',
     'CENSUSAREA': 436.671,
     'fips': 29199},
    'type': 'Feature',
    'geometry': {'type': 'Polygon',
     'coordinates': [[[-91.949747, 40.300832],
       [-92.347801, 40.302756],
       [-92.349897, 40.302733],
       [-92.349975, 40.346505],
       [-92.350807, 40.597273],
       [-92.350776, 40.597274],
       [-92.331445, 40.597714],
       [-92.331205, 40.597805],
       [-92.298754, 40.598469],
       [-92.236484, 40.599531],
       [-92.217603, 40.599832],
       [-92.201669, 40.59998],
       [-92.196162, 40.600069],
       [-92.17978, 40.600529],
       [-92.096387, 40.60183],
       [-92.092875, 40.602082],
       [-92.0832, 40.602244],
       [-92.082339, 40.602176],
       [-92.069521, 40.602772],
       [-92.067904, 40.602648],
       [-92.029649, 40.603713],
       [-91.998683, 40.604433],
       [-91.970988, 40.605112],
       [-91.943114, 40.605913],
       [-91.949747, 40.300832]]]},
    'id': '29199'},
   {'properties': {'GEO_ID': '0500000US29063',
     'STATE': '29',
     'COUNTY': '063',
     'NAME': 'DeKalb',
     'LSAD': 'County',
     'CENSUSAREA': 421.356,
     'fips': 29063},
    'type': 'Feature',
    'geometry': {'type': 'Polygon',
     'coordinates': [[[-94.2188, 40.034885],
       [-94.203457, 40.034539],
       [-94.205844, 39.788954],
       [-94.205884, 39.767152],
       [-94.205432, 39.760821],
       [-94.205728, 39.756815],
       [-94.205961, 39.745841],
       [-94.602257, 39.747195],
       [-94.602819, 39.819608],
       [-94.605377, 40.039217],
       [-94.2188, 40.034885]]]},
    'id': '29063'},
   {'properties': {'GEO_ID': '0500000US29069',
     'STATE': '29',
     'COUNTY': '069',
     'NAME': 'Dunklin',
     'LSAD': 'County',
     'CENSUSAREA': 541.069,
     'fips': 29069},
    'type': 'Feature',
    'geometry': {'type': 'Polygon',
     'coordinates': [[[-90.148398, 36.630656],
       [-89.958786, 36.629331],
       [-89.96131, 36.388803],
       [-89.959375, 35.99902],
       [-89.959377, 35.99902],
       [-89.959893, 35.99902],
       [-89.961075, 35.999135],
       [-89.965327, 35.998813],
       [-89.972563, 35.998994],
       [-90.12635, 35.997596],
       [-90.127331, 35.997635],
       [-90.158812, 35.997375],
       [-90.2888, 35.996419],
       [-90.288947, 35.996418],
       [-90.292376, 35.996397],
       [-90.339434, 35.996033],
       [-90.342616, 35.995895],
       [-90.368718, 35.995812],
       [-90.36443, 36.013625],
       [-90.319168, 36.089976],
       [-90.220425, 36.184764],
       [-90.198735, 36.201382],
       [-90.114922, 36.265595],
       [-90.083731, 36.272332],
       [-90.076301, 36.280708],
       [-90.06398, 36.303038],
       [-90.064514, 36.382085],
       [-90.072897, 36.393007],
       [-90.133993, 36.437906],
       [-90.156369, 36.487748],
       [-90.154409, 36.496832],
       [-90.152481, 36.497952],
       [-90.193943, 36.497823],
       [-90.217323, 36.497797],
       [-90.220702, 36.497858],
       [-90.220732, 36.497858],
       [-90.148398, 36.630656]]]},
    'id': '29069'},
   {'properties': {'GEO_ID': '0500000US29081',
     'STATE': '29',
     'COUNTY': '081',
     'NAME': 'Harrison',
     'LSAD': 'County',
     'CENSUSAREA': 722.504,
     'fips': 29081},
    'type': 'Feature',
    'geometry': {'type': 'Polygon',
     'coordinates': [[[-93.774344, 40.577584],
       [-93.763324, 40.263988],
       [-93.764021, 40.132909],
       [-94.216696, 40.136626],
       [-94.214278, 40.384254],
       [-94.23224, 40.571907],
       [-94.091085, 40.572897],
       [-94.089194, 40.572806],
       [-94.080463, 40.572899],
       [-94.080223, 40.572899],
       [-94.034134, 40.573585],
       [-94.015492, 40.573914],
       [-93.976766, 40.574635],
       [-93.963863, 40.574754],
       [-93.939857, 40.575192],
       [-93.938627, 40.575284],
       [-93.937097, 40.575421],
       [-93.936317, 40.575284],
       [-93.935687, 40.57533],
       [-93.913961, 40.575672],
       [-93.900877, 40.575874],
       [-93.899317, 40.575942],
       [-93.898327, 40.576011],
       [-93.853656, 40.576606],
       [-93.84093, 40.576791],
       [-93.818725, 40.577086],
       [-93.815485, 40.577278],
       [-93.774344, 40.577584]]]},
    'id': '29081'},
   {'properties': {'GEO_ID': '0500000US29089',
     'STATE': '29',
     'COUNTY': '089',
     'NAME': 'Howard',
     'LSAD': 'County',
     'CENSUSAREA': 463.849,
     'fips': 29089},
    'type': 'Feature',
    'geometry': {'type': 'Polygon',
     'coordinates': [[[-92.849224, 39.226218],
       [-92.790369, 39.343586],
       [-92.707367, 39.321614],
       [-92.430229, 39.248795],
       [-92.565124, 38.998274],
       [-92.558086, 38.970692],
       [-92.841617, 38.979821],
       [-92.934569, 39.064547],
       [-92.94165, 39.125892],
       [-92.849224, 39.226218]]]},
    'id': '29089'},
   {'properties': {'GEO_ID': '0500000US29091',
     'STATE': '29',
     'COUNTY': '091',
     'NAME': 'Howell',
     'LSAD': 'County',
     'CENSUSAREA': 927.248,
     'fips': 29091},
    'type': 'Feature',
    'geometry': {'type': 'Polygon',
     'coordinates': [[[-91.658111, 36.888724],
       [-91.674237, 36.787832],
       [-91.672343, 36.499463],
       [-91.686026, 36.499374],
       [-91.687615, 36.499397],
       [-91.726663, 36.499209],
       [-91.766111, 36.499114],
       [-91.784713, 36.499074],
       [-91.7995, 36.498952],
       [-91.80204, 36.498963],
       [-91.805981, 36.498987],
       [-91.864385, 36.498789],
       [-91.865995, 36.498783],
       [-91.985802, 36.498431],
       [-91.988751, 36.498498],
       [-92.019375, 36.498524],
       [-92.028847, 36.498642],
       [-92.055789, 36.49867],
       [-92.057178, 36.49867],
       [-92.074934, 36.498761],
       [-92.098356, 36.498803],
       [-92.120306, 36.498864],
       [-92.120415, 36.498863],
       [-92.112346, 36.794231],
       [-92.095982, 36.794011],
       [-92.088708, 37.056248],
       [-91.655607, 37.048925],
       [-91.657814, 36.889648],
       [-91.658111, 36.888724]]]},
    'id': '29091'},
   {'properties': {'GEO_ID': '0500000US29133',
     'STATE': '29',
     'COUNTY': '133',
     'NAME': 'Mississippi',
     'LSAD': 'County',
     'CENSUSAREA': 411.583,
     'fips': 29133},
    'type': 'Feature',
    'geometry': {'type': 'Polygon',
     'coordinates': [[[-89.519809, 36.869617],
       [-89.465393, 36.935729],
       [-89.322733, 37.009033],
       [-89.29213, 36.992189],
       [-89.278628, 36.98867],
       [-89.271127, 36.985287],
       [-89.261154, 36.982437],
       [-89.245481, 36.98315],
       [-89.226247, 36.985287],
       [-89.207013, 36.982437],
       [-89.185491, 36.973518],
       [-89.170008, 36.970298],
       [-89.132685, 36.9822],
       [-89.100794, 36.944006],
       [-89.100766, 36.943973],
       [-89.147674, 36.847148],
       [-89.177177, 36.835779],
       [-89.178574, 36.816554],
       [-89.179229, 36.812915],
       [-89.12353, 36.785309],
       [-89.123548, 36.78522],
       [-89.130399, 36.751702],
       [-89.142313, 36.755369],
       [-89.169106, 36.759473],
       [-89.184523, 36.753638],
       [-89.197808, 36.739412],
       [-89.201047, 36.725772],
       [-89.19948, 36.716045],
       [-89.171882, 36.672526],
       [-89.174733, 36.650476],
       [-89.174741, 36.650416],
       [-89.187749, 36.641115],
       [-89.197654, 36.628936],
       [-89.200902, 36.618177],
       [-89.202607, 36.601576],
       [-89.213563, 36.580119],
       [-89.227319, 36.569375],
       [-89.236542, 36.566824],
       [-89.259994, 36.565149],
       [-89.27171, 36.571387],
       [-89.294637, 36.593729],
       [-89.313405, 36.62012],
       [-89.326731, 36.632186],
       [-89.327578, 36.632127],
       [-89.519809, 36.869617]]]},
    'id': '29133'},
   {'properties': {'GEO_ID': '0500000US29143',
     'STATE': '29',
     'COUNTY': '143',
     'NAME': 'New Madrid',
     'LSAD': 'County',
     'CENSUSAREA': 674.84,
     'fips': 29143},
    'type': 'Feature',
    'geometry': {'type': 'Polygon',
     'coordinates': [[[-89.485106, 36.497692],
       [-89.47246, 36.513741],
       [-89.465888, 36.529946],
       [-89.465445, 36.536163],
       [-89.467761, 36.546847],
       [-89.473341, 36.559918],
       [-89.479093, 36.568206],
       [-89.484836, 36.571821],
       [-89.500076, 36.576305],
       [-89.527583, 36.581147],
       [-89.546113, 36.579989],
       [-89.55264, 36.577178],
       [-89.566817, 36.564216],
       [-89.571509, 36.552569],
       [-89.570071, 36.544387],
       [-89.560344, 36.525436],
       [-89.5391, 36.498201],
       [-89.522674, 36.481305],
       [-89.519501, 36.475419],
       [-89.509558, 36.375065],
       [-89.513178, 36.359897],
       [-89.519, 36.3486],
       [-89.531822, 36.339246],
       [-89.545006, 36.336809],
       [-89.611308, 36.411273],
       [-89.752147, 36.386396],
       [-89.96131, 36.388803],
       [-89.958786, 36.629331],
       [-89.700952, 36.627453],
       [-89.691603, 36.859517],
       [-89.519809, 36.869617],
       [-89.327578, 36.632127],
       [-89.343753, 36.630991],
       [-89.365548, 36.625059],
       [-89.376367, 36.613868],
       [-89.38814, 36.573416],
       [-89.400581, 36.538337],
       [-89.417293, 36.499033],
       [-89.429311, 36.481875],
       [-89.448468, 36.46442],
       [-89.460436, 36.45814],
       [-89.471718, 36.457001],
       [-89.486215, 36.46162],
       [-89.493198, 36.470124],
       [-89.493495, 36.4787],
       [-89.485106, 36.497692]]]},
    'id': '29143'},
   {'properties': {'GEO_ID': '0500000US29149',
     'STATE': '29',
     'COUNTY': '149',
     'NAME': 'Oregon',
     'LSAD': 'County',
     'CENSUSAREA': 789.796,
     'fips': 29149},
    'type': 'Feature',
    'geometry': {'type': 'Polygon',
     'coordinates': [[[-91.115926, 36.823672],
       [-91.126529, 36.497712],
       [-91.21736, 36.497511],
       [-91.218645, 36.497564],
       [-91.227398, 36.497617],
       [-91.404915, 36.49712],
       [-91.405141, 36.497165],
       [-91.407137, 36.497112],
       [-91.407261, 36.497123],
       [-91.433298, 36.497262],
       [-91.436502, 36.497377],
       [-91.446284, 36.497469],
       [-91.450005, 36.497538],
       [-91.529774, 36.499022],
       [-91.53687, 36.499156],
       [-91.539359, 36.499116],
       [-91.549163, 36.499161],
       [-91.596213, 36.499162],
       [-91.601317, 36.499343],
       [-91.631439, 36.499198],
       [-91.64259, 36.499335],
       [-91.672343, 36.499463],
       [-91.674237, 36.787832],
       [-91.658111, 36.888724],
       [-91.250129, 36.88515],
       [-91.22283, 36.883857],
       [-91.224825, 36.825679],
       [-91.115926, 36.823672]]]},
    'id': '29149'},
   {'properties': {'GEO_ID': '0500000US29155',
     'STATE': '29',
     'COUNTY': '155',
     'NAME': 'Pemiscot',
     'LSAD': 'County',
     'CENSUSAREA': 492.544,
     'fips': 29155},
    'type': 'Feature',
    'geometry': {'type': 'Polygon',
     'coordinates': [[[-89.545006, 36.336809],
       [-89.560439, 36.337746],
       [-89.581636, 36.342357],
       [-89.605668, 36.342234],
       [-89.610689, 36.340442],
       [-89.615841, 36.336085],
       [-89.6198, 36.329546],
       [-89.620255, 36.323006],
       [-89.611819, 36.309088],
       [-89.578492, 36.288317],
       [-89.554289, 36.277751],
       [-89.544797, 36.280458],
       [-89.539487, 36.277368],
       [-89.535529, 36.270541],
       [-89.534745, 36.252576],
       [-89.541621, 36.247891],
       [-89.589561, 36.239116],
       [-89.602374, 36.238106],
       [-89.678046, 36.248284],
       [-89.691308, 36.252079],
       [-89.699817, 36.248384],
       [-89.629452, 36.185382],
       [-89.618228, 36.179966],
       [-89.607004, 36.171179],
       [-89.594397, 36.155457],
       [-89.591605, 36.144096],
       [-89.594, 36.12719],
       [-89.601936, 36.11947],
       [-89.624235, 36.108626],
       [-89.684439, 36.051719],
       [-89.687254, 36.034048],
       [-89.690306, 36.024835],
       [-89.706932, 36.000981],
       [-89.733095, 36.000608],
       [-89.737564, 36.000522],
       [-89.737648, 36.000567],
       [-89.769973, 36.000536],
       [-89.770255, 36.000524],
       [-89.86901, 35.99964],
       [-89.87459, 35.999575],
       [-89.875085, 35.999578],
       [-89.875586, 35.999562],
       [-89.896508, 35.999432],
       [-89.901183, 35.999365],
       [-89.959375, 35.99902],
       [-89.96131, 36.388803],
       [-89.752147, 36.386396],
       [-89.611308, 36.411273],
       [-89.545006, 36.336809]]]},
    'id': '29155'},
   {'properties': {'GEO_ID': '0500000US29165',
     'STATE': '29',
     'COUNTY': '165',
     'NAME': 'Platte',
     'LSAD': 'County',
     'CENSUSAREA': 420.19,
     'fips': 29165},
    'type': 'Feature',
    'geometry': {'type': 'Polygon',
     'coordinates': [[[-94.600819, 39.456155],
       [-94.601733, 39.159603],
       [-94.706723, 39.173801],
       [-94.714137, 39.170403],
       [-94.723637, 39.169003],
       [-94.736537, 39.169203],
       [-94.752338, 39.173203],
       [-94.793948, 39.201276],
       [-94.832399, 39.227218],
       [-94.901009, 39.30168],
       [-94.907569, 39.320623],
       [-94.910641, 39.348335],
       [-94.908337, 39.355647],
       [-94.909954, 39.367459],
       [-94.946662, 39.399717],
       [-94.969061, 39.418876],
       [-95.102888, 39.533347],
       [-94.601326, 39.532138],
       [-94.601246, 39.530372],
       [-94.600819, 39.456155]]]},
    'id': '29165'},
   {'properties': {'GEO_ID': '0500000US29209',
     'STATE': '29',
     'COUNTY': '209',
     'NAME': 'Stone',
     'LSAD': 'County',
     'CENSUSAREA': 464.033,
     'fips': 29209},
    'type': 'Feature',
    'geometry': {'type': 'Polygon',
     'coordinates': [[[-93.610126, 36.99581],
       [-93.337451, 36.992494],
       [-93.342041, 36.817301],
       [-93.304359, 36.816866],
       [-93.315324, 36.498408],
       [-93.315337, 36.498408],
       [-93.394718, 36.498519],
       [-93.396079, 36.498669],
       [-93.426989, 36.498585],
       [-93.507408, 36.498911],
       [-93.514512, 36.498881],
       [-93.584281, 36.498896],
       [-93.584282, 36.498896],
       [-93.5738, 36.820032],
       [-93.611003, 36.922831],
       [-93.610126, 36.99581]]]},
    'id': '29209'},
   {'properties': {'GEO_ID': '0500000US29217',
     'STATE': '29',
     'COUNTY': '217',
     'NAME': 'Vernon',
     'LSAD': 'County',
     'CENSUSAREA': 826.398,
     'fips': 29217},
    'type': 'Feature',
    'geometry': {'type': 'Polygon',
     'coordinates': [[[-94.614055, 38.060056],
       [-94.058285, 38.036627],
       [-94.065675, 37.900987],
       [-94.073524, 37.639722],
       [-94.617575, 37.653577],
       [-94.617576, 37.653671],
       [-94.617734, 37.673105],
       [-94.617734, 37.673127],
       [-94.617885, 37.682214],
       [-94.617687, 37.686653],
       [-94.617651, 37.687671],
       [-94.617805, 37.690178],
       [-94.617975, 37.722176],
       [-94.617808, 37.729707],
       [-94.616862, 37.819456],
       [-94.61645, 37.83756],
       [-94.616426, 37.845282],
       [-94.616, 37.863126],
       [-94.615834, 37.87251],
       [-94.615921, 37.878331],
       [-94.615706, 37.886843],
       [-94.615469, 37.901775],
       [-94.615393, 37.906392],
       [-94.615181, 37.915944],
       [-94.614778, 37.9342],
       [-94.614835, 37.9367],
       [-94.614754, 37.940769],
       [-94.614612, 37.944362],
       [-94.614594, 37.949978],
       [-94.614562, 37.951517],
       [-94.614557, 37.971037],
       [-94.614465, 37.987799],
       [-94.614212, 37.992462],
       [-94.613981, 38.036949],
       [-94.613981, 38.037057],
       [-94.614055, 38.060056]]]},
    'id': '29217'},
   {'properties': {'GEO_ID': '0500000US29227',
     'STATE': '29',
     'COUNTY': '227',
     'NAME': 'Worth',
     'LSAD': 'County',
     'CENSUSAREA': 266.613,
     'fips': 29227},
    'type': 'Feature',
    'geometry': {'type': 'Polygon',
     'coordinates': [[[-94.214278, 40.384254],
       [-94.598487, 40.386714],
       [-94.632035, 40.571186],
       [-94.632032, 40.571186],
       [-94.594001, 40.570966],
       [-94.542154, 40.570809],
       [-94.541828, 40.570809],
       [-94.538318, 40.570763],
       [-94.537058, 40.570763],
       [-94.533878, 40.570739],
       [-94.48928, 40.570707],
       [-94.471213, 40.570825],
       [-94.470648, 40.57083],
       [-94.460088, 40.570947],
       [-94.429725, 40.571041],
       [-94.358307, 40.571363],
       [-94.336706, 40.571452],
       [-94.336556, 40.571475],
       [-94.324765, 40.571477],
       [-94.310724, 40.571524],
       [-94.294813, 40.571341],
       [-94.28735, 40.571521],
       [-94.23224, 40.571907],
       [-94.214278, 40.384254]]]},
    'id': '29227'},
   {'properties': {'GEO_ID': '0500000US29013',
     'STATE': '29',
     'COUNTY': '013',
     'NAME': 'Bates',
     'LSAD': 'County',
     'CENSUSAREA': 836.689,
     'fips': 29013},
    'type': 'Feature',
    'geometry': {'type': 'Polygon',
     'coordinates': [[[-94.065713, 38.447087],
       [-94.075214, 38.214502],
       [-94.049895, 38.213985],
       [-94.058285, 38.036627],
       [-94.614055, 38.060056],
       [-94.614055, 38.060088],
       [-94.614089, 38.065901],
       [-94.614061, 38.067343],
       [-94.613856, 38.149769],
       [-94.613748, 38.160633],
       [-94.613422, 38.167908],
       [-94.613073, 38.190552],
       [-94.612848, 38.200714],
       [-94.612822, 38.203918],
       [-94.612658, 38.217649],
       [-94.612659, 38.219251],
       [-94.612635, 38.226987],
       [-94.612614, 38.237766],
       [-94.612692, 38.270394],
       [-94.612849, 38.289914],
       [-94.612844, 38.291423],
       [-94.612673, 38.302527],
       [-94.612673, 38.314832],
       [-94.612788, 38.320142],
       [-94.612825, 38.324387],
       [-94.613, 38.335801],
       [-94.613312, 38.364407],
       [-94.613329, 38.369618],
       [-94.613275, 38.388718],
       [-94.613265, 38.392426],
       [-94.613365, 38.403422],
       [-94.612866, 38.477571],
       [-94.612865, 38.477602],
       [-94.31217, 38.471496],
       [-94.065713, 38.447087]]]},
   'id': '29013'}]}]

  


  //windrose
  public windrose:any;
  public windrose_data:any;



  public selectcntyfip:number;
  public locationscntyfips:number;



  //task -- analysis

  public evaluation:boolean=false;
  public task1:boolean=true;
  public task2:boolean=false;
  public task3:boolean=false;
  public task4:boolean=false;
  public task5:boolean=false;
  public task6:boolean=false;

  public exploration:boolean=false;


  public task1_start:boolean=true;
  public task1_stop:boolean=false;

  public task2_start:boolean=false;
  public task2_stop:boolean=false;

  public task3_start:boolean=false;
  public task3_stop:boolean=false;

  public task4_start:boolean=false;
  public task4_stop:boolean=false;

  public task5_start:boolean=false;
  public task5_stop:boolean=false;

  public task6_start:boolean=false;
  public task6_stop:boolean=false;

  public exploration_start:boolean=false;
  public exploration_stop:boolean=false;

  public survey:boolean=false;

  public userid:any;

  public current_task:any='Not Started'

  public black_out:boolean=false;



  constructor(public http: HttpClient,private spinner: NgxSpinnerService,public route: ActivatedRoute,private router: Router) {
    

   }

  ngOnInit(): void {
    this.selectcntyfip=29003;
    this.locationscntyfips=null;



   this.counties=[{'cnty_fips': 29003, 'cnty_name': 'Andrew'},
   {'cnty_fips': 29005, 'cnty_name': 'Atchison'},
   {'cnty_fips': 29009, 'cnty_name': 'Barry'},
   {'cnty_fips': 29011, 'cnty_name': 'Barton'},
   {'cnty_fips': 29013, 'cnty_name': 'Bates'},
   {'cnty_fips': 29021, 'cnty_name': 'Buchanan'},
   {'cnty_fips': 29025, 'cnty_name': 'Caldwell'},
   {'cnty_fips': 29037, 'cnty_name': 'Cass'},
   {'cnty_fips': 29039, 'cnty_name': 'Cedar'},
   {'cnty_fips': 29047, 'cnty_name': 'Clay'},
   {'cnty_fips': 29049, 'cnty_name': 'Clinton'},
   {'cnty_fips': 29057, 'cnty_name': 'Dade'},
   {'cnty_fips': 29061, 'cnty_name': 'Daviess'},
   {'cnty_fips': 29063, 'cnty_name': 'DeKalb'},
   {'cnty_fips': 29075, 'cnty_name': 'Gentry'},
   {'cnty_fips': 29081, 'cnty_name': 'Harrison'},
   {'cnty_fips': 29083, 'cnty_name': 'Henry'},
   {'cnty_fips': 29087, 'cnty_name': 'Holt'},
   {'cnty_fips': 29095, 'cnty_name': 'Jackson'},
   {'cnty_fips': 29097, 'cnty_name': 'Jasper'},
   {'cnty_fips': 29101, 'cnty_name': 'Johnson'},
   {'cnty_fips': 29107, 'cnty_name': 'Lafayette'},
   {'cnty_fips': 29109, 'cnty_name': 'Lawrence'},
   {'cnty_fips': 29119, 'cnty_name': 'McDonald'},
   {'cnty_fips': 29145, 'cnty_name': 'Newton'},
   {'cnty_fips': 29147, 'cnty_name': 'Nodaway'},
   {'cnty_fips': 29165, 'cnty_name': 'Platte'},
   {'cnty_fips': 29177, 'cnty_name': 'Ray'},
   {'cnty_fips': 29185, 'cnty_name': 'St. Clair'},
   {'cnty_fips': 29217, 'cnty_name': 'Vernon'},
   {'cnty_fips': 29227, 'cnty_name': 'Worth'},
   {'cnty_fips': 29015, 'cnty_name': 'Benton'},
   {'cnty_fips': 29017, 'cnty_name': 'Bollinger'},
   {'cnty_fips': 29023, 'cnty_name': 'Butler'},
   {'cnty_fips': 29029, 'cnty_name': 'Camden'},
   {'cnty_fips': 29031, 'cnty_name': 'Cape Girardeau'},
   {'cnty_fips': 29035, 'cnty_name': 'Carter'},
   {'cnty_fips': 29043, 'cnty_name': 'Christian'},
   {'cnty_fips': 29055, 'cnty_name': 'Crawford'},
   {'cnty_fips': 29059, 'cnty_name': 'Dallas'},
   {'cnty_fips': 29065, 'cnty_name': 'Dent'},
   {'cnty_fips': 29067, 'cnty_name': 'Douglas'},
   {'cnty_fips': 29069, 'cnty_name': 'Dunklin'},
   {'cnty_fips': 29077, 'cnty_name': 'Greene'},
   {'cnty_fips': 29085, 'cnty_name': 'Hickory'},
   {'cnty_fips': 29091, 'cnty_name': 'Howell'},
   {'cnty_fips': 29093, 'cnty_name': 'Iron'},
   {'cnty_fips': 29099, 'cnty_name': 'Jefferson'},
   {'cnty_fips': 29105, 'cnty_name': 'Laclede'},
   {'cnty_fips': 29123, 'cnty_name': 'Madison'},
   {'cnty_fips': 29125, 'cnty_name': 'Maries'},
   {'cnty_fips': 29131, 'cnty_name': 'Miller'},
   {'cnty_fips': 29133, 'cnty_name': 'Mississippi'},
   {'cnty_fips': 29143, 'cnty_name': 'New Madrid'},
   {'cnty_fips': 29149, 'cnty_name': 'Oregon'},
   {'cnty_fips': 29153, 'cnty_name': 'Ozark'},
   {'cnty_fips': 29155, 'cnty_name': 'Pemiscot'},
   {'cnty_fips': 29157, 'cnty_name': 'Perry'},
   {'cnty_fips': 29161, 'cnty_name': 'Phelps'},
   {'cnty_fips': 29167, 'cnty_name': 'Polk'},
   {'cnty_fips': 29169, 'cnty_name': 'Pulaski'},
   {'cnty_fips': 29179, 'cnty_name': 'Reynolds'},
   {'cnty_fips': 29181, 'cnty_name': 'Ripley'},
   {'cnty_fips': 29186, 'cnty_name': 'Ste. Genevieve'},
   {'cnty_fips': 29187, 'cnty_name': 'St. Francois'},
   {'cnty_fips': 29201, 'cnty_name': 'Scott'},
   {'cnty_fips': 29203, 'cnty_name': 'Shannon'},
   {'cnty_fips': 29207, 'cnty_name': 'Stoddard'},
   {'cnty_fips': 29209, 'cnty_name': 'Stone'},
   {'cnty_fips': 29213, 'cnty_name': 'Taney'},
   {'cnty_fips': 29215, 'cnty_name': 'Texas'},
   {'cnty_fips': 29221, 'cnty_name': 'Washington'},
   {'cnty_fips': 29223, 'cnty_name': 'Wayne'},
   {'cnty_fips': 29225, 'cnty_name': 'Webster'},
   {'cnty_fips': 29229, 'cnty_name': 'Wright'},
   {'cnty_fips': 29001, 'cnty_name': 'Adair'},
   {'cnty_fips': 29007, 'cnty_name': 'Audrain'},
   {'cnty_fips': 29019, 'cnty_name': 'Boone'},
   {'cnty_fips': 29027, 'cnty_name': 'Callaway'},
   {'cnty_fips': 29033, 'cnty_name': 'Carroll'},
   {'cnty_fips': 29041, 'cnty_name': 'Chariton'},
   {'cnty_fips': 29045, 'cnty_name': 'Clark'},
   {'cnty_fips': 29051, 'cnty_name': 'Cole'},
   {'cnty_fips': 29053, 'cnty_name': 'Cooper'},
   {'cnty_fips': 29071, 'cnty_name': 'Franklin'},
   {'cnty_fips': 29073, 'cnty_name': 'Gasconade'},
   {'cnty_fips': 29079, 'cnty_name': 'Grundy'},
   {'cnty_fips': 29089, 'cnty_name': 'Howard'},
   {'cnty_fips': 29103, 'cnty_name': 'Knox'},
   {'cnty_fips': 29111, 'cnty_name': 'Lewis'},
   {'cnty_fips': 29113, 'cnty_name': 'Lincoln'},
   {'cnty_fips': 29115, 'cnty_name': 'Linn'},
   {'cnty_fips': 29117, 'cnty_name': 'Livingston'},
   {'cnty_fips': 29121, 'cnty_name': 'Macon'},
   {'cnty_fips': 29127, 'cnty_name': 'Marion'},
   {'cnty_fips': 29129, 'cnty_name': 'Mercer'},
   {'cnty_fips': 29135, 'cnty_name': 'Moniteau'},
   {'cnty_fips': 29137, 'cnty_name': 'Monroe'},
   {'cnty_fips': 29139, 'cnty_name': 'Montgomery'},
   {'cnty_fips': 29141, 'cnty_name': 'Morgan'},
   {'cnty_fips': 29151, 'cnty_name': 'Osage'},
   {'cnty_fips': 29159, 'cnty_name': 'Pettis'},
   {'cnty_fips': 29163, 'cnty_name': 'Pike'},
   {'cnty_fips': 29171, 'cnty_name': 'Putnam'},
   {'cnty_fips': 29173, 'cnty_name': 'Ralls'},
   {'cnty_fips': 29175, 'cnty_name': 'Randolph'},
   {'cnty_fips': 29183, 'cnty_name': 'St. Charles'},
   {'cnty_fips': 29189, 'cnty_name': 'St. Louis County'},
   {'cnty_fips': 29195, 'cnty_name': 'Saline'},
   {'cnty_fips': 29197, 'cnty_name': 'Schuyler'},
   {'cnty_fips': 29199, 'cnty_name': 'Scotland'},
   {'cnty_fips': 29205, 'cnty_name': 'Shelby'},
   {'cnty_fips': 29211, 'cnty_name': 'Sullivan'},
   {'cnty_fips': 29219, 'cnty_name': 'Warren'},
   {'cnty_fips': 29510, 'cnty_name': 'St. Louis City'}]
   
   this.county_name=this.counties[0].cnty_name;
   this.county_fips=this.counties[0].cnty_fips;
  

    this.route.queryParams
    .subscribe(params => {
      this.locationscntyfips=Number(params.fips)
      this.userid=params.userid;
      console.log('USERIDUSERIDUSERID!***@*#*@#')
      console.log(this.userid)

    });

    if(this.locationscntyfips!=null && isNaN(this.locationscntyfips)==false){
      this.selectcntyfip=this.locationscntyfips
      let temp=this.counties.find(e=> e['cnty_fips']===this.locationscntyfips)
      this.county_name=temp.cnty_name;
      this.county_fips=this.locationscntyfips
    }
    this.spinner.show();

    map_status=0
    risk_map = L.map("risk_map").setView([38.573936, -92.603760], 6.2);


    this.getStatsBar(this.selectcntyfip);
    this.getRiskFactors(this.selectcntyfip,this.current_risk_factor);
    this.factorsMapData(this.current_risk_factor);
    this.getCovidData(this.selectcntyfip);
    this.getWindrose
    this.getWindrosedata(this.selectcntyfip);
    this.getCovidDate();


  }
  





  // ---------- Task Analysis --------------//


  start_task1(task_id:any){

    this.sendClick(task_id,'start')
    this.task1_start=false;
    this.task1_stop=true;
    this.current_task=task_id;
    this.black_out=false;




  }

  stop_task1(task_id:any){
    this.task1_stop=false;
    this.task1=false;
    this.task2=true;
    this.task2_start=true;
    console.log(task_id);
    this.sendClick(task_id,'end')

    this.black_out=true;

  }

  start_task2(task_id:any){
    this.sendClick(task_id,'start')
    this.task2_start=false;
    this.task2_stop=true;
    console.log(task_id);
    this.current_task=task_id;
    this.black_out=false;
  }

  stop_task2(task_id:any){
    this.sendClick(task_id,'end')
    this.task2_stop=false;
    this.task2=false;
    this.task3=true;
    this.task3_start=true;
    console.log(task_id);
    this.black_out=true;

  }

  start_task3(task_id:any){
    this.sendClick(task_id,'start')
    this.task3_start=false;
    this.task3_stop=true;
    console.log(task_id);
    this.current_task=task_id;
    this.black_out=false;
  }

  stop_task3(task_id:any){
    this.sendClick(task_id,'end')
    this.task3_stop=false;
    this.task3=false;
    this.task4=true;
    this.task4_start=true;
    console.log(task_id);
    this.black_out=true;

  }

  start_task4(task_id:any){
    this.sendClick(task_id,'start')
    this.task4_start=false;
    this.task4_stop=true;
    console.log(task_id);
    this.current_task=task_id;
    this.black_out=false;
  }

  stop_task4(task_id:any){
    this.sendClick(task_id,'end')
    this.task4_stop=false;
    this.task4=false;
    this.task5=true;
    this.task5_start=true;
    console.log(task_id);
    this.black_out=true;

  }

  start_task5(task_id:any){
    this.sendClick(task_id,'start')
    this.task5_start=false;
    this.task5_stop=true;
    console.log(task_id);
    this.current_task=task_id;
    this.black_out=false;
  }

  stop_task5(task_id:any){
    this.sendClick(task_id,'end')
    this.task5_stop=false;
    this.task5=false;
    this.task6=true;
    this.task6_start=true;
    console.log(task_id);
    this.black_out=true;

  }


  start_task6(task_id:any){
    this.sendClick(task_id,'start')
    this.task6_start=false;
    this.task6_stop=true;
    console.log(task_id);
    this.current_task=task_id;
    this.black_out=false;
  }

  stop_task6(task_id:any){
    this.sendClick(task_id,'end')
    this.task6_stop=false;
    this.task6=false;
    this.exploration=true;
    this.exploration_start=true;
    console.log(task_id);
    this.black_out=true;

  }

  start_exploration(task_id:any){
    this.sendClick(task_id,'start')
    this.exploration_start=false;
    this.exploration_stop=true;
    console.log(task_id);
    this.current_task=task_id;
    this.black_out=false;
  }

  stop_exploration(task_id:any){
    this.sendClick(task_id,'end')
    this.exploration_stop=false;
    this.exploration=false;
    this.survey=true;
    console.log(task_id);
    this.black_out=false;

  }

  getClickEvent(divname:string){
    console.log(divname)
    this.sendClick(this.current_task,divname)
  }


  startSurvey(){
    this.router.navigate(['/survey2'], { queryParams: { userid: this.userid}})
  }

  sendClick(task:any,event:any){

    let click_data={
      userID:this.userid,
      event:event,
      taskID:task
    }


    const customheaders= new HttpHeaders()
    .set('Content-Type', 'application/json');

    this.http.post(environment.base_url+"/countiesclicks",JSON.stringify(click_data), {headers: customheaders}).subscribe(
    response=> {

      console.log(response)
 
    },
    error => {
      console.log(error)
    }
    )
    
  }



// get Data for Selected Risk Factor
  getFactorsData(factor:any){
    this.current_risk_factor=factor.target.value
    this.current_factor='total'

    geoJSON.clearLayers();

    this.getClickEvent(factor.target.value+'_buttonclick');

    this.getRiskFactors(this.county_fips,factor.target.value);
    this.factorsMapData(factor.target.value);
  }


  //trigger data pull and visual creation for new selected county -- by drop down 
  getCountyData(fips:any){

    console.log('GET COUNTY DATA')
    console.log(fips)
    this.spinnertogg2=true;
    this.county_fips=Number(fips.target.value);
    this.current_factor='total'

    let temp=this.counties.find(e=> e['cnty_fips']===Number(fips.target.value))
    this.county_name=temp.cnty_name


  

    geoJSON.clearLayers();
    this.map();
    this.getStatsBar(Number(fips.target.value))
    this.getRiskFactors(Number(fips.target.value),this.current_risk_factor);
    this.getCovidData(Number(fips.target.value));
    this.getWindrosedata(Number(fips.target.value));

    this.getClickEvent('countyselectclick')

  }
 
//trigger data pull and visual creation for new selected county -- by map 
  getCountyDataClick(event){
    this.getClickEvent('mapclick')
    this.spinnertogg2=true;
    this.county_fips=Number(event.target.feature.properties.fips);
    this.current_factor='total'

   
  

    let temp=this.counties.find(e=> e['cnty_fips']===Number(event.target.feature.properties.fips))
    this.county_name=temp.cnty_name


    geoJSON.clearLayers();
    this.map();
    this.getStatsBar(Number(event.target.feature.properties.fips))
    this.getRiskFactors(Number(event.target.feature.properties.fips), this.current_risk_factor);
    this.getCovidData(Number(event.target.feature.properties.fips));
    this.getWindrosedata(Number(event.target.feature.properties.fips));
    
  
    this.selectcntyfip=this.county_fips;


  }


// get county quick facts data
  getStatsBar(covid_fips:any){
    const customheaders= new HttpHeaders()
          .set('Content-Type', 'application/json');

    this.http.post(environment.base_url+"/getquickstats",JSON.stringify(covid_fips), {headers: customheaders}).subscribe(
      response=> {
       
        this.county_pop=response[0];
        this.county_65=response[1];
        this.county_cov_case=response[2];
        this.county_cov_deaths=response[3];
        this.county_total_risk=response[4];



      },
      error => {
        console.log(error)
      }
    )

  }


//pull data for risk factor bar plots and build plots
  getRiskFactors(fips:any, factor:any){
    const customheaders= new HttpHeaders()
          .set('Content-Type', 'application/json');

    this.http.post(environment.base_url+"/getriskfactordata",JSON.stringify([fips,factor]), {headers: customheaders}).subscribe(
      response=> {
        console.log(response)
        this.risk_factors=response;
        this.RiskFactorsbarplot();

      },
      error => {
        console.log(error)
      }
    )

  }


// Get data for covid plots and build plots
  getCovidData(covid_fips:any){
    const customheaders= new HttpHeaders()
          .set('Content-Type', 'application/json');

    this.http.post(environment.base_url+"/getcovidcasesdeaths",JSON.stringify(covid_fips), {headers: customheaders}).subscribe(
      response=> {
        this.covid=response;
        this.getCovidPlots();

        if (this.initial_covid_plots==true)
        {
          this.spinnertogg=false;
          this.initial_covid_plots=false;
        }
        else
        {
          this.spinnertogg2=false;
        }



      },
      error => {
        console.log(error)
      }
    )

  }


  
// function to create risk factor bar plots
  RiskFactorsbarplot(){
    this.risk_factors_bars = [];

    for(let i=0; i<this.risk_factors.length; i++){
      this.risk_factors_bars.push({
        data: [
          {
            x: [Number(this.risk_factors[i].factors_values)],
            y: [String(this.risk_factors[i].factors)+'      '],
            //y:[""],
            orientation: 'h',
            type: 'bar',
            width:2,
            marker: {
              color: this.risk_factors[i].Q5_color
            },
          },
         ],
         layout: {

          title: {
            text: this.risk_factors[i].description,
            font: {
              size: 12
            },
            standoff: -5
          },
          plot_bgcolor: '#f5f6f9',
           xaxis:{
            range: [Number(this.risk_factors[i].min_2), Number(this.risk_factors[i].max)],
          // dtick: this.temp[i].max,
          tickvals: [Number(this.risk_factors[i].mean).toFixed(2),Number(this.risk_factors[i].max).toFixed(3)],
          

           },
          autosize:true, 
          height: 45, 
         // margin:{l:100, r:25, t:15, b:15, pad:1},
          margin:{l:3, r:25, t:15, b:15, pad:0},
          size: 6,
          shapes: [{
            name:'test',
            type: 'line',
            x0: this.risk_factors[i].mean,
            y0: -1,
            x1: this.risk_factors[i].mean,
            y1: 1,
            line: {
              color: '#676767',
              width: 1,
              dash: 'dot'
            }
          }],
        }
      })
    }  
}



//Create Covid County and State plots
  getCovidPlots(){

    //// Covid county cases plots
    this.covid_county_cases = {
      data: [
          { x: this.covid[0], 
            y: this.covid[1], 
            type: 'bar', 
            //mode: 'lines', 
            name:'county cases',
            marker: {color: '#676767'} 
          },
      ],
      layout: {
              autosize:true, 
              height: 100, 

              title: {
                text: this.county_name+' County Weekly Cases',
                font: {
                  size: 12
                },
                standoff: -5
              },
  
              margin:{
                l:50, 
                r:20, 
                t:20, 
                b:30, 
                pad:0
              },

              size: 6,
              xaxis: {
                type: 'date',
              }
         }
     };

     this.covid_county_deaths= {
      data: [
          { x: this.covid[0], 
            y: this.covid[2], 
            type: 'bar', 
            //mode: 'lines', 
            name:'county deaths',
            marker: {color: '#d9d9d9'} 
          },
      ],
      layout: {
              autosize:true, 
              height: 100, 

              title: {
                text: this.county_name+' County Weekly Deaths',
                font: {
                  size: 12
                },
                standoff: -5
              },
  
              margin:{
                l:50, 
                r:20, 
                t:20, 
                b:30, 
                pad:0
              },

              size: 6,
              xaxis: {
                type: 'date',

              }
         }
     };



     if(this.initial_covid_plots==true){
             ///MO covid cases plots
    this.covid_cases = {
      data: [
          { x: this.covid[0], 
            y: this.covid[3], 
            type: 'bar', 
            //mode: 'lines', 
            name:'county cases',
            marker: {color: '#676767'} 
          },
      ],
      layout: {
              autosize:true,
              height:100,

              title: {
                automargin: true,
                text:'Missouri Total Weekly Cases',
                font: {
                  size: 12
                },
                standoff: 2
              },
  
              margin:{
                l:50, 
                r:20, 
                t:20, 
                b:30, 
                pad:0
              },

              size: 6,
              xaxis: {
                type: 'date',
              }
         }
     };

    //// MO covid deaths plots
    this.covid_deaths = {
      data: [
          { x: this.covid[0], 
            y: this.covid[4], 
            type: 'bar', 
            //mode: 'lines', 
            name:'county cases',
            marker: {color: '#d9d9d9'} 
          },
      ],
      layout: {
              autosize:true, 
              height: 100, 

              title: {
                text:'Missouri Total Weekly Deaths',
                font: {
                  size: 12
                },
                standoff: -5
                
              },
  
              margin:{
                l:50, 
                r:20, 
                t:20, 
                b:30, 
                pad:0
              },

              size: 6,
              xaxis: {
                type: 'date',
    
              }
         }
     };

     }





  }




//Update Covid Plots
  updateCovidPlots(){

    //// Covid county cases plots
    this.covid_county_cases = {
      data: [
          { x: this.covid[0], 
            y: this.covid[1], 
            type: 'scatter', 
            mode: 'lines', 
            name:'county cases',
            marker: {color: 'orange'} 
          },
      ],
      layout: {
              width: 400, 
              height: 150, 

              title: {
                text: this.county_name+' County Weekly Cases',
                font: {
                  size: 12
                },
              },
  
              margin:{
                l:50, 
                r:20, 
                t:50, 
                b:30, 
                pad:0
              },

              size: 6,
              xaxis: {
                type: 'date',
                rangeselector: {
                  buttons: [
                            {
                              count: 1,
                              label: '1M',
                              step: 'month',
                              stepmode: 'backward'
                            },
                            {
                              count: 6,
                              label: '6M',
                              step: 'month',
                              stepmode: 'backward'
                            },
                            {step: 'all'}
                  ]
                },
              }
         }
     };

     this.covid_county_deaths= {
      data: [
          { x: this.covid[0], 
            y: this.covid[2], 
            type: 'scatter', 
            mode: 'lines', 
            name:'county deaths',
            marker: {color: 'red'} 
          },
      ],
      layout: {
              width: 400, 
              height: 150, 

              title: {
                text: this.county_name+' County Weekly Deaths',
                font: {
                  size: 12
                },
              },
  
              margin:{
                l:50, 
                r:20, 
                t:50, 
                b:30, 
                pad:0
              },

              size: 6,
              xaxis: {
                type: 'date',
                rangeselector: {
                  buttons: [
                            {
                              count: 1,
                              label: '1M',
                              step: 'month',
                              stepmode: 'backward'
                            },
                            {
                              count: 6,
                              label: '6M',
                              step: 'month',
                              stepmode: 'backward'
                            },
                            {step: 'all'}
                  ]
                },
              }
         }
     };
  }




  getCovidDate(){
    const customheaders= new HttpHeaders()
          .set('Content-Type', 'application/json');
  
    this.http.get(environment.base_url+"/getCovidDate", {headers: customheaders}).subscribe(
      response=> {

       
      this.update_date=response;
      
      console.log(this.update_date)
  
  
      },
      error => {
        console.log(error)
      }
    )
  
  }





// Get data for covid plots and build plots
getWindrosedata(covid_fips:any){
  const customheaders= new HttpHeaders()
        .set('Content-Type', 'application/json');

  this.http.post(environment.base_url+"/getwindrose",JSON.stringify(covid_fips), {headers: customheaders}).subscribe(
    response=> {
      this.windrose_data=response;
      this.getWindrose()
   


    },
    error => {
      console.log(error)
    }
  )

}





// Create WindRose chart
  getWindrose(){
    this.windrose = {
      data: [
          { 
            r: [Number(this.windrose_data[0])],
            theta: ["%Susceptibility Risk"],
            marker: {color: "#019A1E"},
            showlegend:false,
            type: "barpolar",
          },
          {
            r: [Number(this.windrose_data[3])],
            theta: ["%Socioeconomic Risk",],
            marker: {color: "#470544"},
            showlegend:false,
            type: "barpolar"
          },
          {
            r: [Number(this.windrose_data[1])],
            theta: ["%Transmission Risk",],
            marker: {color: "#1041a2"},
            showlegend:false,
            type: "barpolar"
          },
          {
            r: [Number(this.windrose_data[2])],
            theta: ["%Exposure Risk",],
            marker: {color: "#676767"},
            showlegend:false,
            type: "barpolar"
          },
          {
            r: [Number(this.windrose_data[4])],
            theta: ["%Accessibility Risk",],
            marker: {color: "#A60306"},
            showlegend:false,
            type: "barpolar"
          },
          {
            r: [Number(this.windrose_data[5])],
            theta: ["%Health Resources Risk",],
            marker: {color: "#FB6D00"},
            showlegend:false,
            type: "barpolar"
          }
        ],
        
        layout:{
         // title: "County Risk Assessment",
         // font: {size: 16},
          width: 150, 
          height: 100, 
        //  margin: {t:0,r:0,b:0,l:20},
          //plot_bgcolor: 'rgba(245,246,249,0)',
          //paper_bgcolor:'black',
          //plot_bgcolor:'black',

          polar:{
            barmode:"overlay",
            bgcolor:'black',
            bargap:0,
            radialaxis: {ticksuffix: "%", angle: 45, dtick: 20},
            angularaxis: {direction: "clockwise"}
          }
        },


    }
}



factorsMapData(risk:any){
  const customheaders= new HttpHeaders()
        .set('Content-Type', 'application/json');

  this.http.post(environment.base_url+"/factorsMapData",JSON.stringify(risk), {headers: customheaders}).subscribe(
    response=> {

     
    this.factors=response;
    //this.factors_meta=response[1];

    this.map();


    },
    error => {
      console.log(error)
    }
  )

}






// -------------------- MAP FUNCTIONS ------------------ //


triggerFactor(factor, index){

  this.getClickEvent(factor+'_riskfactorclick')
  this.current_factor=factor[0];
  geoJSON.clearLayers();
  this.map();


  this.current_description=this.risk_factors[index].description;


}



  map(){

      var data=this.factors;
      var display=this.current_factor+' risk value'
     // var metadata=this.factors_meta;
      var current_fact=this.current_factor+'_Q5';



      var current_risk_factor=this.current_risk_factor;
      

     // let factor_max=metadata.find(e=> e['factor']===current_fact)

     // var min=0;
      //if (factor_max.min<0){
      //  min=-2;
      //}
    
    //   var threshold=200;

    //   var colorrange=colormap({
    //     colormap:'portland',~
    //     nshades:threshold,
    //     format: 'hex',
    //     alpha: 1
    // })

    
    var current=this.county_fips

    //var ranges= getBin(min,factor_max.max,200);

    L.tileLayer("http://{s}.tile.openstreetmap.org/{z}/{x}/{y}.png", 
		{
			id: "mapbox.light",
			attribution: "SOS"
			// can have min and max zoom here
    }).addTo(risk_map);


    // info block
    info = new L.Control({position: 'bottomleft'});
    info.onAdd = function () {
      if(map_status==0){
        this._div = L.DomUtil.create("div", "info");
        map_status=1;
      }
      else
      {

        this._div=document.getElementsByClassName("info")[0];

      }
        this.update();
        return this._div;
      };

     //info box display
      info.update = function (props: any) {
        this._div.innerHTML =
          (props ? "<b>County: </b>"+props.NAME+"<br><b>"+display+":</b>"+getInfo(props['fips'])+"</b><br />" : "<h4>Click to see details of another county.</h4>");
      };
      info.addTo(risk_map);

    //data look up for info box
      function getInfo(fips){
        let temp=data.find(e=> e['cnty_fips']===String(fips))
        return temp[current_fact]
      }


    //geoJSON object and coloring of county
		geoJSON= L.geoJSON(this.map_obj, {
      style: function (feature) {
        return{
          color: 'black',
          weight: getLineWidth(feature.id),
          //fillColor:getcolor(feature.id),
          fillColor:getcolorQ5(feature.id),
          fillOpacity:0.8,
        }
      },
      onEachFeature: (feature, layer) => this.onEachFeature(feature, layer)
    }).addTo(risk_map);
  


//get color of county



      // Functions for color and threshold 
      function getBin(min,max,threshold)
      {
      
        var multiple=max/threshold
        var bins=[];
        for (var i=min; i<max; i+=multiple)
        {
          bins.push(i)
        }
      
        return bins
      }



    function getcolorQ5(value){


      let temp=data.find(e=> e['cnty_fips']===value)
      let palette=[]

      if (current_risk_factor=='accessibility'){
        palette=['#640204', '#af0406', '#c7384b', '#e580a3', '#ffb3ed']
      }
      else if (current_risk_factor=='healthresources'){
        palette=['#4d2100','#b34d00','#ff6f00','#ffb84d','#fff5b3']
      }
      else if (current_risk_factor=='exposure'){
        palette=['#404040', '#676767','#8c8c8c', '#b3b3b3','#d9d9d9']
      }
      else if (current_risk_factor=='socioeconomic'){
        palette=['#470544','#7b2b88','#a05fb9','#a192d3','#b5c6fc']
      }
      else if (current_risk_factor=='susceptibility'){
        palette=['#004c0e','#019A1E','#83ec13','#e5fc4f','#fbffb3']
      } 
      else if (current_risk_factor=='transmission'){
        palette=['#000f4d','#0e388b','#51a4c8','#8fd1d6','#b5fdf8']
      }
    


      if (Number(temp[current_fact])<=1){
        return palette[4]
      }
      else if (Number(temp[current_fact] ==2)){
        return palette[3]
      }
      else if (Number(temp[current_fact]==3)){
        return palette[2]
      }
      else if (Number(temp[current_fact] ==4)){
        return palette[1]
      }
      else {
        return palette[0]
      }



    }
   
    // function getcolor3(value){


    //   let temp=data.find(e=> e['cnty_fips']===value)
		// 	var color;


		// 	//light to dark
				
		// 	for(var i=(threshold-1); i>=min; i--)
		// 	{
		// 		if(temp[current_fact] >=ranges[i])
		// 		{
		// 			color= colorrange[i];
		// 			break;
		// 		}
		// 	}

		// 	return color
		// }



    function getLineWidth(fips){
      if (fips==current){
          return 4
      }
      else{
        return 0.5
      }
    }



  }


  

// map functions 
  highlightFeature(e) {
    const layer = e.target;
    layer.setStyle({
      weight: 2,
      color: "black",
      fillOpacity: 1.0
    });
    if (!L.Browser.ie && !L.Browser.edge) {
      //layer.bringToFront();
    }
    info.update(layer.feature.properties);
  }

   onEachFeature(feature, layer: L.Layer) {
    layer.on({
      mouseover: this.highlightFeature,
      mouseout: this.resetHighlight,
      click: (event) => this.getCountyDataClick(event),
    });
  }

  resetHighlight(e) {
    geoJSON.resetStyle(e.target);
    info.update();
  }

  /*zoomToFeature(e) {
    risk_map.fitBounds(e.target.getBounds());
  }*/

  onOutlineEachFeature(feature, layer: L.Layer) {
    layer.on({
    });
  }




}